# NCERT: 11TH_PHYSICS_CH04_LAWS_OF_MOTION_CH04_LAWS_OF_MOTION_EN
...
the force by the child cancels the force (friction) by
the floor. The corollary is: if there were no friction,
...
the table, the normal force R. Ris a self-adjusting
...
changes from v to v + ∆vi.e. its initial momentum
p = mv changes by Ap = m∆v . According to the
Second Law,
...
of 1 m s² to a mass of 1 kg. This unit is known as
newton: 1 N = 1 kg m s².

Let us note at this stage some important points
about the second law :

### 1. In the second law, F = 0 implies a = 0. The second
...
Chapter 6. Any internal forces in the system
...
Fig. 4.5 Acceleration at an instant is determined by
...
Example 4.2 A bullet of mass 0.04 kg
...
force. In the history of science, impulsive forces were
put in a conceptually different category from
ordinary forces. Newtonian mechanics has no such
distinction. Impulsive force is like any other force –
except that it is large and acts for a short time.
...
of two bodies as a whole, F FAB and F are
...
body or a system of particles (See Chapter 6).
...
Case (b)
(Px) )initial initial = mucos 30°, (Py) initial
mu cos 30°, (Py) final
...
x-component of impulse = −2 mu_cos_30
y-component of impulse = 0
...
At. According to the second law, F At is the change
in momentum of the bullet and -F At is the
change in momentum of the gun. Since initially,
both are at rest, the change in momentum equals
the final momentum for each. Thus if p₁ is the
momentum of the bullet after firing and pois the
recoil momentum of the gun, p= -Pi.e. P♭ + Pg
= 0. That is, the total momentum of the (bullet +
...
initial momenta p₁ and pƉ. The bodies collide,
get apart, with final momenta p' and p'B
...
concurrent forces F1, F2 and F requires that
...
third force, F. As seen in Fig. 4.7, the three
...
F2.... F if they can be represented by the sides
...
Answer Figures 4.8(b) and 4.8(c) are known as
free-body diagrams. Figure 4.8(b) is the free-body
diagram of W and Fig. 4.8(c) is the free-body
diagram of point P.
...
the action of three forces - the tensions T₁ and
T₂, and the horizontal force 50 N. The horizontal
...
force F is written as F = - kx where x is the
...
Tthroughout the string. This assumption is true
for a string of negligible mass.

We learnt that there are four fundamental
forces in nature. Of these, the weak and strong
forces appear in domains that do not concern
us here. Only the gravitational and electrical
forces are relevant in the context of mechanics.
The different contact forces of mechanics
mentioned above fundamentally arise from
electrical forces. This may seem surprising
...
Fig. 4.10 Static and sliding friction: (a) Impending
...
force F increases, ƒ also increases, remaining
...
value of static friction (fs) may is independent of
...
contact. The constant µ¸ is called the coefficient
...
If the applied force F exceeds (fs) max the body

begins to slide on the surface. It is found experi-
mentally that when relative motion has started,
the frictional force decreases from the static

maximum value (fs) max. Frictional force that

opposes relative motion between surfaces in
contact is called kinetic or sliding friction and is
denoted by f. Kinetic friction, like static fric-
...
where με the coefficient of kinetic friction,
depends only on the surfaces in contact. As
mentioned above, experiments show that με is
less than μ. When relative motion has begun,
the acceleration of the body according to the
second law is (F-f₁₄)/m. For a body moving with
constant velocity, F = f. If the applied force on
the body is removed, its acceleration is -f/m
...
static frictional force f opposing the impending
...
mg sin e = f mg cos 0 = N

و

As e increases, the self-adjusting frictional force
f increases until at 0 = 0 max f achieves its
maximum value, (fs) max = με Ν.
...
= tan¯¹ μ
με or θ
When @ becomes just a little more than 0тах,
there is a small net force on the block and it
begins to slide. Note that depends only on
μand is independent of the mass of the block.
...
10 0 
m s²). Neglect the mass of the string.
...
speed vis ²/R directed towards the centre.
...
This shows that for a given value of µ and R,
...
reduce the contribution of friction to the
...
Comparing this with Eq. (4.18) we see that
...
is not needed. The optimum speed vis given by
1 speed
Eq. (4.22):
...
equation also tells you that for v < v, frictional
force will be up the slope and that a car can be
parked only if tan 0 ≤ μ.
...
slip is given by Eq. (4.18) :

v² ≤ μRg

Now, R = 3 m, g = 9.8 m s¯², μ = 0.1. That is,
μ¸Rg = 2.94 m² s¯². v = 18_km/h = 5 m s¯¹; i.e.,
v² = 25 m² s¯². The condition is not obeyed.
The cyclist will slip while taking the
circular turn.
...
0.1 ms-2. What is the action of the block
...
Rof the floor on the block. By the First Law,
...
downwards with 0.1 m s². The free-body
...
(d) Fat a point at a certain instant determines a at the same point at that instant.
...
tangent to the surface in contact. Static friction f opposes impending relative
motion; kinetic friction f opposes actual relative motion. They are independent
...
μ. (co-efficient of static friction) and µ₁ (co-efficient of kinetic friction) are
constants characteristic of the pair of surfaces in contact. It is found
experimentally that uis less than μ.
...
### 4. In the second law of motion F = ma, F stands for the net force due to all
...
6. Static friction is a self-adjusting force up to its limit με Ν (f≤ μ N). Do not put
f = μ N without being sure that the maximum value of static friction is coming
into play.
...
(d) lying on the floor of a train which is accelerating with 1 m s², the stone
...
to
3.5 m s¹ in 25 s. The direction of the motion of the body remains
...
acceleration of 5.0 m s². Calculate the initial thrust (force) of the blast.

### 4.10 A body of mass 0.40 kg moving initially with a constant speed of 10 m s¯¹ to
...
body at that time to be x = 0, and predict its position att = −5 s, 25 s, 100 s.

### 4.11 A truck starts from rest and accelerates uniformly at 2.0 m s¯². At t = 10 s, a
...
### 4.18 Two billiard balls each of mass 0.05 kg moving in opposite directions with speed 6 m s¯¹

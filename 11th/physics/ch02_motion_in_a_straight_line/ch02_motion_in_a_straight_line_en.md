# NCERT: 11TH_PHYSICS_CH02_MOTION_IN_A_STRAIGHT_LINE_CH02_MOTION_IN_A_STRAIGHT_LINE_EN
...
of
of 
the object is much smaller than the distance it moves in a
...
At becomes infinitesimally small. In other words,
...
where the symbol Ato stands for the operation
of taking limit as to of the quantity on its
...
position with respect to time, at that instant.
...
velocity at time t = 4s (point P) for the motion
...
slope of line P₁P2 (Fig. 2.1) gives the value of
average velocity over the interval 3s to 5 s.

Table

1 2

Now, we decrease the value of At from 2 s to 1
s. Then line P₁P₂ becomes 9192 and its slope
gives the value of the average velocity over
the interval 3.5 s to 4.5 s. In the limit At → 0,
the line P₁P₂ becomes tangent to the position-
...
in Fig. 2.1, x = 0.08 ť. Table 2.1 gives the
value of Ax/At calculated for At equal to 2.0 s,
...
corresponding values of x, i.e. x (t₁) = 0.08 tỉ

and x (t₂) = 0.08 t3. The sixth column lists the
difference Ax = x (t₂)
x (t) and the last
column gives the ratio of Ax and At, i.e. the
average velocity corresponding to the value
of At listed in the first column.

We see from Table 2.1 that as we decrease
the value of At from 2.0 s to 0.010 s, the value of
...
Then, we calculate ∆x/At from the data for
...
moving along x-axis is given by_x = a + bt2
where a = 8.5 m, b = 2.5 m s¯² and tis
...
+ 24.0 m s¯¹ and a velocity of – 24.0 m s¯¹
both have an associated speed of 24.0 m s¯¹. It
...
or simply velocities at time t₂ and t₁. It is the
...
Velocity-time graph for motions with
...
direction at time t₁. Between times 0 to
t₁, it moves in positive x direction
...
and base T. Therefore, area = u x T = uT which
...
From Eq. (2.4), t = (v – v)/a. Substituting this in
Eq. (2.7a), we get
...
connecting five quantities u, v, a, tand x. These
are kinematic equations of rectilinear motion for
constant acceleration.

The set of Eq. (2.9a) were obtained by
assuming that at t = 0, the position of the particle,
xis 0. We can obtain a more general equation if
we take the position coordinate at t = 0 as non-
zero, say x. Then Eqs. (2.9a) are modified
(replacing x by x − x ) to :
...
Example 2.2 Obtain equations of motion
...
The advantage of this method is that it can be used
for motion with non-uniform acceleration
also.
...
upwards with a velocity of 20 m s¯¹ from
...
calculate the corresponding time taken t₁ and
...
Solving, we get t₂ = 3 s

Therefore, the total time taken by the ball before
it hits the ground = t₁ + t₂ =2s+3s = 5 s.
...
equal to 9.8 m s¯². Free fall is thus a case of
...
υο= = 0 and the equations of motion become:
...
Example 2.5 Galileo's law of odd
...
equal intervals and find out the distances
...
(-1/2) gt² as yo— the position coordinate after
first time interval t, then third column gives
the positions in the unit of y. The fourth
column gives the distances traversed in
successive ts. We find that the distances are
...
depends on the initial velocity (v) and the
...
before it stops be d. Then, using equation of
motion_v² = v¸² + 2 ax, and noting that v = 0, we
...
Therefore, v = 0, and a = - g = = − g = −9.8 m s-2. The
distance travelled d and the reaction time tare
related by
...
Given d = 21.0 cm and g= 9.8 m s¯² the reaction
time is
...
as the time interval At becomes infinitesimally small :
...
### 5. The area under the velocity-time curve between times t₁ and t₂ is equal to the displacement
...
x, time taken t, initial velocity v, final velocity v and acceleration a are related
...
if the position of the object at time t = 0 is 0. If the particle starts at x x = x, x in above
...
returns home by an auto with a speed of 25 km h¯¹. Choose suitable scales and
...
A car moving along a straight highway with speed of 126 km h¯¹ is brought to a
...
(c) Choose the x = 0 m and t = 0s to be the location and time of the ball at its
...
player's hands? (Take g = 9.8 m s¯² and neglect air resistance).
...
would not like to tell the tired m

# NCERT: 11TH_PHYSICS_CH11_THERMODYNAMICS_CH11_THERMODYNAMICS_EN
...
schematically in Fig. 11.1 (a). In this case, it is
found that any possible pair of values (P₁, V₁) will
be in equilibrium with any possible pair of values
(PB, V). Next, suppose that the adiabatic wall is
...
situation is shown in Fig. 11.1(b). The pressure
and volume variables of the two gases change to
(PB', V') and (P₁', V₁₄ ') such that the new states
...
Fig. 11.3 (a) Internal energy U of a gas is the sum
...
Uis simply a macroscopic variable of the system.
...
(gas) (see section 11.7). If we neglect the small
...
i.e. the energy (AQ) supplied to the system goes

in partly to increase the internal energy of the
system (AU) and the rest in work on the
environment (AW). Equation (11.1) is known as
...
(P, V) to (P₂, V₂), we can first change the
volume of the gas from V₁ to V₂, keeping its
pressure constant i.e. we can first go the state
(P₁, V₂) and then change the pressure of the
gas from P to P₂, keeping volume constant, to
take the gas to (P₂, V₂). Alternatively, we can
first keep the volume constant and then keep
the pressure constant. Since U is a state
variable, AU depends only on the initial and
final states and not on the path taken by the
gas to go from one to the other. However, ∆Q
and AW will, in general, depend on the path
taken to go from the initial to final states. From
the First Law of Thermodynamics, Eq. (11.2),
it is clear that the combination AQ – AW, is
however, path independent. This shows that
if a system is taken through a process in which
AU = 0 (for example, isothermal expansion of
...
by the system against a constant pressure Pis
...
As an application of Eq. (11.3), consider the
...
pressure, 1 g of water has a volume 1 cm³ in
liquid phase and 1671 cm³ in vapour phase.
Therefore,

AW=P(V-V) = 1.013×105 × (1671 ×10°)=169.2J
...
Suppose an amount of heat AQ supplied to a
substance changes its temperature from T to
T+ AT. We define heat capacity of a substance
(see Chapter 10) to be
...
We expect AQ and, therefore, heat capacity S
...
mass of the substance min kg:

231

If the amount of substance is specified in
terms of moles μ (instead of mass min kg), we
...
the substance. Likes, Cis independent of the
...
(See Section 12.5 and 12.6). Consider a solid of
N atoms, each vibrating about its mean
position. An oscillator in one dimension has
average energy of 2 × 2 k｡T = k₁T. In three
dimensions, the average energy is 3 kT.
For a mole of a solid, the total energy is

A

U = 3 k㎏T × N₁ = 3 RT (•: k｡T × N₁ = R)
Now, at constant pressure, ∆Q = ∆U + P AV =
AU, since for a solid A Vis negligible. Therefore,
AQ
C =
ΔΤ
...
specific heat capacity is J kg¯¹ K-1.
...
is 4186 J kg¯¹ K¯¹ i.e. 4.186 J g¯¹ K¯¹. The so
...
(11.8)
where C and C are molar specific heat
capacities of an ideal gas at constant pressure
and volume respectively and Ris the universal
...
Equations (11.9) to (11.11) give the desired
relation, Eq. (11.8).
...
pressure are not uniform [Fig. 11.6(b)].
...
Fig. 11.6 (a) The partition in the box is suddenly
...
and Vor Tand V. The pressure-volume curve
...
temperature T, and density pare intensive
...
extensive quantity AV is extensive.)
...
of gases undergoing an explosive chemical
reaction, mentioned in section 11.7 are also
...
Tto T', the system achieves the temperature T'.
...
Table 11.2 Some special thermodynamic
processes
...
Fig. 11.7 In a quasi-static process, the temperature
...
For an isothermal process (Tfixed), the ideal gas
equation gives
...
temperature T) from its initial state (P₁, V₁) to
the final state (P2, V₂). At any intermediate stage
with pressure P and volume change from V to
V+AV(AVsmall)

AW = PAV

Taking (AV→ 0) and summing the quantity
AW over the entire process,
...
the ideal gas equation PV= μ RTand_taken the
...
for V₂ > V₁, W> 0; and for V₂ < V₁, W< 0. That
...
(11.13)
where y is the ratio of specific heats (ordinary
...
state (P, V, T₁) to the state (P₂, V, T₂).
...
From Eq. (11.14), the constant is P₁V or P₂V₂
...
T₂ < T₁. On the other hand, if work is done on
the gas (W < 0), we get T₂ > T₁ i.e., the
...
In an isochoric process, Vis constant. No work
is done on or by the gas. From Eq. (11.1), the
...
In an isobaric process, Pis fixed. Work done by
the gas is
...
a fundamental limitation to the efficiency of a
heat engine and the co-efficient of
performance of a refrigerator. In simple terms,
it says that efficiency of a heat engine can
never be unity. For a refrigerator, the Second
Law says that the co-efficient of performance
can never be infinite. The following two
statements, one due to Kelvin and Planck
...
from the surroundings and performs work Won
...
is not possible. The spontaneous processes of
nature are irreversible. Several examples can be
cited. The base of a vessel on an oven is hotter
than its other parts. When the vessel is removed,
heat is transferred from the base to the other
parts, bringing the vessel to a uniform
temperature (which in due course cools to the
temperature of the surroundings). The process
cannot be reversed; a part of the vessel will not
get cooler spontaneously and warm up the base.
It will violate the Second Law of Thermodynamics,
if it did. The free expansion of a gas is irreversible.
The combustion reaction of a mixture of petrol
and air ignited by a spark cannot be reversed.
Cooking gas leaking from a gas cylinder in the
kitchen diffuses to the entire room. The diffusion
process will not spontaneously reverse and bring
the gas back to the cylinder. The stirring of a
liquid in thermal contact with a reservoir will
convert the work done into heat, increasing the
internal energy of the reservoir. The process
cannot be reversed exactly; otherwise it would
amount to conversion of heat entirely into work,
...
reservoirs at temperatures T₁ and T₂? It turns
...
T₁ and a cold reservoir at temperature T₂. What
...
T₁ absorbing heat Q₁ from the hot reservoir, and
another isothermal process at temperature T
releasing heat Q₂ to the cold reservoir. To
complete a cycle, we need to take the system
from temperature T₁ to T₂ and then back from
temperature T₂ to T₁. Which processes should
...
temperature range T₁₂ to T₁ to ensure that at each
...
Carnot cycle for a heat engine with an
...
(a) Step1 → 2 Isothermal expansion of the gas
taking its state from (P₁, V₁, T₁) to
(P2, V2, T₁).

The heat absorbed by the gas (Q₁) from the
reservoir at temperature T₁ is given by
Eq. (11.12). This is also the work done (W₁→2)
...
gas from (P3, V3, T₂) to (P4, V4, T₂).

3'

Heat released (Q2) by the gas to the reservoir

at temperature T, is given by Eq. (11.12). This

is also the work done (W3 →4) on the gas
...
From Eqs. (11.18) to (11.21) total work done
...
step of the Carnot cycle given in Fig. 11.9 can
be reversed. This will amount to taking heat Q2
from the cold reservoir at T₂, doing work Won
...
and T of the hot and cold reservoirs respectively,
...
(Carnot) engine Rand an irreversible engine I
...
Let I absorb heat Q₁ from the source, deliver
work W' and release the heat Q₁- W to the sink.
We arrange so that Rreturns the same heat Q₁
to the source, taking heat Q₂ from the sink and
requiring work_W = Q₁ – Q₂ to be done on it.
...
than that of Ii.e. W< W' for a given Q₁. With R
acting like a refrigerator, this would mean
Q₂ = Q₁ - W> Q₁ – W'. Thus, on the whole,
the coupled I-R system extracts heat
(Q₁ – W) – (Q₁ – W ') = (W' – W) from the cold
...
Thermodynamics. Hence the assertion η₁ > η
...
a Carnot engine given by Eq. (11.27) is
...
in the calculation of efficiency n of a Carnot
engine. The ideal gas has a simple equation of
state, which allows us to readily calculate n, but
...
gas temperature introduced in section 11.9.
...
Wis the work done by the system and AU

where AQ is the heat supplied to the system,
...
where u is the number of moles of the substance. For a solid, the law of equipartition
of energy gives
...
the ideal gas equation PV = μ RT) is a relation connecting different state variables.
...
8. In an isothermal expansion of an ideal gas from volume V₁ to V at temperature T the
heat absorbed (G) equals th the work done (W) by the gas, each given by
...
Work done by an ideal gas in an adiabatic change of state from (P₁, V₁, T₁) to (P₂, V2, T₂)
...
12. Carnot engine is a reversible engine operating between two temperatures T, (source) and
T, (sink). The Carnot cycle consists of two isothermal processes connected by two
...
mass of N2 = 28; R = 8.3 J mol¯¹ K¯¹.)

11.3 Explain why

(a) Two bodies at different temperatures T₁ and T2 if brought in thermal contact do
not necessarily settle to the mean temperature (T₁ + T2)/2.

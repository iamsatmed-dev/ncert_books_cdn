# NCERT: 11TH_PHYSICS_CH13_OSCILLATIONS_CH13_OSCILLATIONS_EN
...
relation between v and Tis

v = 1/T

(13.1)

The unit of v is thus s¯¹. After the discoverer of
...
If the argument of this function, wt, is
...
consider a sine function, f(t) = A sin ot. Further,
a linear combination of sine and cosine functions
like,

f(t) = A sin wt + B cos ot
(13.3c)
...
Here D and 4 are constant given by
...
value, sin ot has a period T= 2π/ω ; cos 2 wt
has a period π/ω =T/2; and sin 4 wt has a
period 2π/4ω= T/4. The period of the first
...
sin æt + cos ot is a periodic function, it can
...
curves 3 and 4 are for $ = 0 and -π/4
...
Fig. 13.8 Plots of Eq. (13.4) for = 0 for two different

periods.

be re

Example 13.3 Which of the following
...
the vector OP at t = 0 makes an angle of $ with
...
Example 13.4 The figure given below
...
will make an angle of wt + with the +ve
...
We can take projection of the motion of Pon
...
(a) At t= t = 0, OP makes an angle of 45° = π/4 rad
...
shown explicitly. For example, sin(15°) means sine of 15 degree, but sin(15) means sine of 15 radians.
...
90° = with the x-axis. After a time t, it
2
covers an angle of 2 It in the clockwise
...
comparing with Eq. (13.4). We find that this
...
x-axis. Eq. (13.9) gives the instantaneous
velocity of a particle executing SHM, where
displacement is given by Eq. (13.4). We can, of
...
circular motion has a magnitude ²/A or w²A,
...
motion is its angular speed @ times the radius
of the circle A.
...
Fig. 13.11 The velocity, v (t), of the particle P' is
...
Fig. 13.12 The acceleration, a(t), of the particle P' is
...
For simplicity, let us put ¢ = 0 and write the
...
from -ωΑ to A and a(t) from –²A to w²A. With
...
(5.0 m)(2π s¯¹) sin [(2π s¯¹) ×1.5 s

=

+ π/4]

= − (5.0 m)(2πs¯¹) sin [(3π + π/4)]
...
13.6 FORCE LAW FOR SIMPLE HARMONIC
MOTION
...
Fig. 13.13 Displacement, velocity and acceleration of
a particle in simple harmonic motion have
the same period T, but they differ in phase

Example 13.5 A body oscillates with SHM
...
= 2n s¯¹ and its time period T= 1 s.
At t = 1.5 s

(a)

displacement = (5.0 m) cos [(2π s¯¹)×
1.5 s + π/4]
...
its force law. Going from Eq. (13.4) to Eq. (13.13)
required us to differentiate two times. Likewise,
by integrating the force law Eq. (13.13) two times,
...
to x2, x³, etc. These then are called non-linear
oscillators.
...
-kx (force exerted by the spring on
...
position. Note, since the sign of vis immaterial
...
forces. The spring force F = –kxis a conservative
...
kinetic; at the extremes x = ±A, it is all
potential energy. In the course of motion
between these limits, kinetic energy increases
at the expense of potential energy or
...
oscillator is thus independent of time as
expected for motion under any conservative
force. The time and displacement dependence
of the potential and kinetic energies of a
linear simple harmonic oscillator are shown
in Fig. 13.16.

Example 13.7 A block whose mass is 1 kg
is fastened to a spring. The spring has a
spring constant of 50 N m¯¹. The block is
...
frequency, as as given by Eq. (13.14b), is
...
total energy remains constant at all tor x.
...
K.E. is zero and hence the total energy
...
position. (b) The radial force T-mg cose
...
through the support. Fig. 13.17(a) shows this
system. Fig. 13.17(b) is a kind of ‘free-body'
diagram of the simple pendulum showing the
forces acting on the bob.

Let @ be the angle made by the string with
...
resolved into the component mg cose along the
...
a radial acceleration (L) and also a tangental
...
T-mg cose, while the tangential acceleration is
provided by mg sine. It is more convenient to
work with torque about the support since the
radial force gives zero torque. Torque tabout
...
about the support and a is the angular
acceleration. Thus,
...
Table 13.1 sin e as ma function of angle 0
...
Equation (13.24) is mathematically, identical to
Eq. (13.11) except that the variable is angular
displacement. Hence we have proved that for
small q, the motion of the bob is simple harmonic.
...
mL². Eq. (13.25) then gives the well-known
...
We can simplify Eq. (13.22) if we

the displacement is small. We know that sin 0
can be expressed as,
...
Example 13.8 What is the length of a
simple pendulum, which ticks seconds?

Answer From Eq. (13.26), the time period of a
...
value of the function sin 0. From this table it
can be seen that for 6 as large as 20 degrees,
sin e is nearly the same as e expressed
in radians.
...
The motion that repeats itself is called periodlic motion.

The period Tis the time reequired for one complete oscillation, or cycle. It is related to
the frequency v by,
...
in which A is the amplitude of the displacement, the quantity (ot + 4) is the phase of
the motion, and 4 is the phase constant. The angular frequency @ is related to the
...
motion are periodic functions, having the velocity amplitude v =ω A and acceleration
...
The period Tis the least time after which motion repeats itself. Thus, motion repeats
itself after nTwhere n is an integer.
...
as well as due to simple harmonic force in two dimensions equal to: -mr. In the
latter case, the phases of motion, in two perpendicular directions (x and y) must differ
by π/2. Thus, for example, a particle subject to a force -m²r with initial position (0,
A) and velocity (@A, 0) will move uniformly in a circle of radius A.

For linear simple harmonic motion with a given w, two initial conditions are necessary
...
From point 4 above, given amplitude or energy, phase of motion is determined by the
initial position or initial velocity.

A combination of two simple harmonic motions with arbitrary amplitudes and phases
...
The period of SHM does not depend on amplitude or energy or the phase constant.
...
For motion of a particle to be simple harmonic, its displacement x must be expressible
in either of the following forms :

x = A cos wt + B sin ot

x = A cos (wt + a), x = B sin (ωt + β)
...
A swimmer completing one (return) trip from one bank of a river to the other
and back.
...
Which of the following examples represent (nearly) simple harmonic motion and
...
motion of a ball bearing inside a smooth curved bowl, when released from a
...
A particle is in linear simple harmonic motion between two points, A and B, 10 cm
...
If the initial (t = 0) position of the particle is 1 cm and its initial velocity is a cm/s,
what are its amplitude and initial phase angle ? The angular frequency of the particle
is π s¯¹. If instead of the cosine function, we choose the sine function to describe the
SHM: : x = B sin (wt + a), what are the amplitude and initial phase of the particle
...
of earth is 3.5s? (gon the surface of earth is 9.8 m s¯²)

13.16 A simple pendulum of length I and having a bob of mass Mis suspended in a car.
...
density p. The cork is depressed slightly and then released. Show that the cork
...
other end to atmosphere. A small pressure difference is maintained

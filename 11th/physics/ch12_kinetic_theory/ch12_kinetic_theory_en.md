# NCERT: 11TH_PHYSICS_CH12_KINETIC_THEORY_CH12_KINETIC_THEORY_EN
...
temperature in kelvin or (absolute) scale. Kis a
constant for the given sample but varies with
the volume of the gas. If we now bring in the
idea of atoms or molecules, then Kis proportional
to the number of molecules, (say) N in the
sample. We can write K = Nk . Observation tells
us that this kis same for all gases. It is called
...
if P, Vand Tare same, then Nis also same for all
...
is 6.02 × 1023. This is known as Avogadro number
and is denoted by N. The mass of 22.4 litres of
...
k₁ is a universal constant. The temperature Tis
absolute temperature. Choosing kelvin scale for
absolute temperature, R = 8.314 J_mol-¹K-1.
...
molecules, M is the molar mass and N the
...
wheren is the number density, i.e. number of
molecules per unit volume. k₁ is the Boltzmann
...
If we fix u and Tin Eq. (12.3), we get
PV = constant
...
ideal gases: μ₁ moles of gas 1, μ₂ moles of gas 2,
...
kg m³. The density of water vapour at 100 °C
and 1 atm pressure is 0.6 kg m³. The
...
water = 1000 kg m-³. To estimate the volume of
...
obeys the gas law. Since Vand Tare common to
the two gases, we have_P₁V= μ ₁ RT and P₂V=
μ₂ RT, i.e. (P₁/P₂) = (μ₁ / μ₂). Here 1 and 2 refer
...
(i) By definition μ₁ = (N₁/N₁ ) and µ₂ = (N₂/N_)
where N₁ and N₂ are the number of molecules
of 1 and 2, and N is the Avogadro's number.
Therefore, (N₁/N₂) = (μ₁ / μ₂) = 3/2.
(ii) We can also write μ₁ = (m₁/M₁) and μ₂
(m2/M₂) where m₁ and m₂ are the masses of
1 and 2; and M₁ and M₂ are their molecular
masses. (Both m₁ and M₁; as well as m₂ and
M₂ should be expressed in the same units).
If p₁ and p₂ are the mass densities of 1 and
...
V1/3 or 10 times, i.e., 10 ×2Å = 20 Å. So the
average distance is 2 × 20 = 40_Å.
...
12.4 ΚΙΝETIC THEORY OF AN IDEAL GAS
...
its y and zcomponents of velocity do not change
in the collision but the x-component reverses
sign. That is, the velocity after collision is
(-V, V, V₂). The change in momentum of the
molecule is: -mv - (mv) = 2mv. By the
...
velocity (vx, v, v₂) hitting the wall in time At is
12A v At n, where n is the number of molecules
...
transfer Q/At and pressure is force per unit
...
where v is the average of v². Now the gas
...
where vis the speed and 2 denotes the mean
...
At, a molecule with x-component of velocity v
will hit the wall if it is within the distance v At
from the wall. That is, all molecules within the
volume Av At only can hit the wall in time At.
...
At do not appear in the final result. By Pascal's
...
number of molecules hitting the wall in time At
was found to be ½ n Av At. Now the collisions
are random and the gas is in a steady state.
Thus, if a molecule with velocity (v, v, v₂)
...
after a collision acquires the velocity (v, v, v₂).
...
The square root of v² is known as root mean

square (rms) speed and is denoted by v
(We can also write U2 as < v² >.)
Vrms
516 m s¹
...
in the mixture. Equation (12.14) becomes
...
in air. It follows from Eq. (12.19) that at the same
...
(3/2) kT. It depends only on temperature, and
...
(ii) Now 12 m v = average kinetic energy per
molecule = (3/2)) kTwhere m is the mass
...
where Mdenotes the molecular mass of the gas.
...
energy = ½ m <² > is constant. So smaller the
...
consisting of translational energy & and
rotational energy &
...
motion, e.g., ½ mv2 and similar terms in
vand v. In, Eq. (12.24) we see that in thermal
...
Fig. 12.6 The two independent axes of rotation of
a diatomic molecule

where @₁ and ₂ are the angular speeds about
the axes 1 and 2 and I, I, are the corresponding
...
average energy is ½ k₁T. The most elegant
...
theoretically. Later, we shall also discuss briefly,
...
Tis (3/2)kT. The total internal energy of a mole
of such a gas is
...
Note that C - C₁ = R is true for any ideal
...
specific heats of several gases given in Table 12.2.
...
specific heat at constant volume, C₁ = (3/2) R,
and molar specific heat at constant pressure,
C₁ = (3/2) R + R = (5/2) R. Since the volume of
...
average energy of 2 × ½ k㎏T = k㎏T . In three
dimensions, the average energy is 3 k㎏T. For a
...
Now at constant pressure ∆Q = ∆U + PAV

= ∆U, since for a solid AV is negligible. Hence,
...
volume πď <v> ∆t wherein any other molecule
will collide with it (see Fig. 12.7). If n is the
number of molecules per unit volume, the
molecule suffers ηπď <v> ∆t collisions in time
At. Thus the rate of collisions is nπď <v> or the
...
collisions, called the mean free path I, is :
1 = <v> τ = 1/(ηπď)
...
Let us estimate I and t for air molecules with
...
for a water molecule in water vapour at 373 Κ.
...
So n=2.7×1025 x
=2×1025 m
Hence, mean free path 1=4×10~7m
Note that the mean free path is 100 times the
interatomic distance ~ 40 Å = 4 × 10-9m calculated
...
where u is the number of moles and Nis the number of molecules. Rand k₁ are universal
...
where n is number density of molecules, m the mass of the molecule and 2 is the
...
fixed temperature the heavier molecule has the lower average
...
absorption, the energy in each mode being equal to 12 k, T. Each translational and
rotational degree of freedom corresponds to one energy mode of absorption and has
energy 12 kg T. Each vibrational frequency has two modes of energy (kinetic and potential)
...
The mean free path lis the average distance covered by a molecule between two successive
collisions :
...
in thermal equilibrium is 1½ k T. Each quadratic term in the total energy expression of
...
kinetic energy ½ mv of the molecules.

< 2 > is not always equal to ( < v>)2. The average of a squared quantity is not necessarily
...
region of the plot) ? (Molecular mass of H2 = 2.02 u, of O₂ = 32.0 u,
R = 8.31 J mol-1 K-1.)
...
oxygen taken out of the cylinder (R = 8.31 J mol-1 K-¹, molecular mass of O₂ = 32 u).

12.5 An air bubble of volume 1.0 cm³ rises from the bottom of a lake 40 m deep at a
...
vapour and other constituents) in a room of capacity 25.0 m³ at a temperature of
ature
...
12.10 Estimate the mean free path and collision frequency of a nitrogen molecule in a
...
molecule moves freely between between two successive collisions (Molecular mass of
N₂ = 28.0 u).

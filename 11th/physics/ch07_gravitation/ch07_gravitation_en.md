# NCERT: 11TH_PHYSICS_CH07_GRAVITATION_CH07_GRAVITATION_EN
...
Polish monk named Nicolaus Copernicus (1473-1543)
...
of the ellipse (Fig. 7.la). This law was a
...
fixed at F₁ and F₂. The tip of a pencil holds
...
Semi-major axis in units of 1010 m.
...
9 = The quotient (T2/a³) in units of
...
at the perihelion Pin Fig. 7.1(a) be vp and
the Sun-planet distance SP be rp. Relate
{Ip, Vp} to the corresponding quantities at
the aphelion {ra, VA}. Will the planet take
equal times to traverse BAC and CPB?

Answer The magnitude of the angular
momentum at Pis Lp = mprpvp, since inspection
tells us that r, and v, are mutually
perpendicular. Similarly, LA = mprava. From
...
planet of mass m in time interval At is (Fig.
7.2) AA given by
...
time period T by the relation V = 2πR / T. The
time period T is about 27.3 days and R was
already known then to be about 3.84 10°m. If
we substitute these numbers in Eq. (7.3), we get
a value of a much smaller than the value of
...
earth, we will have a aRm²; ga R2 and we get
...
in agreement with a value of g = 9.8 m s¯² and
the value of a from Eq. (7.3). These observations
...
vertices of the A ABC. A mass 2m is placed
at the centroid G.
...
FR = 2Gm² j+ 2Gm² (-i cos 30° -j sin 30°)
( –

+ 2Gm² i cos 30° – j sin 30°) = 0
...
experiment. S₁ and S₂ are large spheres
...
proportional to θ, equal to τθ. Where • is the
restoring couple per unit angle of twist. ° can be
...
If Lis the length of the bar AB, AB then the
...
Observation of e thus enables one to
...
Fig. 7.7 m is in a mine located at a depth
...
situated at a distancer from the centre. The
point Plies outside the sphere of radius r. For
...
Plies on the surface. This smaller sphere
therefore exerts a force on a mass mat P as if
its mass Mis concentrated at the centre. Thus
...
density and hence its mass is ME = Rp

where Mis the mass of the earth R₁ is its radius
and p is the density. On the other hand the
mass of the sphere M of radius ris
...
earth, then_r = R and the gravitational force on
it is, from Eq. (7.10)
...
Acceleration g is readily measurable. Ris a
...
estimate M from Eq. (7.12). This is the reason
...
(R+h). If F (h) denoted the magnitude of
...
GME
surface of earth : 9 = p2 For h << R』, we can
R
...
at the centre. If M is the mass of the smaller
sphere, then,
...
Fig. 7.8 (b) gat a depth d. In this case only the smaller
sphere of radius (R-d) contributes to g.
...
point at a height h₁ from the surface of the earth
and another point vertically above it at a height
h₂ from the surface, the work done in lifting the
...
(7.22)
in moving the particle is just
energy between its
final and initial positions.Observe that the
constant W cancels out in Eq. (7.22). Setting h
= 0 in the last equation, we get W(h = 0 ) = W
...
particle and rits distance from the centre of the
earth. If we now calculate the work done in
lifting a particle from r= r₁ to r = r₂ (r₂ > r₁) along
...
so that once again W12 W(r₂) - W(r₁).
Setting r = infinity in the last equation, we get
W(r= infinity) = W₁. Thus, W₁ is the potential
...
V. The energy of an object is the sum of potential
and kinetic energy. As before W₁ denotes that
...
centre of the earth (R = radius of the earth), its
...
The minimum value of V corresponds to the
...
Using the relation g = GME /R, we get
...
(V)min≈11.2 km/s. This is called the escape
...
of a distance (R + h) from the centre of the earth,
where R = radius of the earth. If m is the mass
...
traverses a distance 2π(R+ h) with speed V. Its
...
be neglected in comparison to R in Eq. (7.38).
...
g = GM/R². In every orbit, the satellite
...
in a circular orbit with speed vis
...
6.67×10-11 × (27.3 3 × × 24 24 × × 60 60 × × 60)2 60) 2
...
k = 10-13 s2 m³. The moon is at a distance
of 3.84 x 105 km from the earth. Obtain its
...
orbit of radius 2 RE about the Earth. How much
...
any two particles of masses mandatonal force
...
ational constant, which has the value 6.672

where G is the universal ×10-11 N m² kg-2.
2. If we have to find the resultant gravitational force acting on the particle m due to a number of
masses M1, M2, ....Metc. we use the principle of superposition. Let F1, F2, ....F be the individual
forces due to M1, M2, ....Mn, each given by the law of gravitation. From the principle of superposition
each force acts independently and uninfluenced by the other bodies. The resultant force Fris
then found by vector addition

FR = F + F + ...... + F = F.

where the symbol 'E' stands for summation.
...
where M is the mass of the Sun. Most planets have nearly circular orbits about the Sun. For
...
where Vis taken to be zero at r → ∞. The total potential energy for a system of particles is the
...
### 3. In Kepler's third law (see Eq. (7.1) and T2 = K R³. The constant Ks is the same for all
...
This choice implies that V→0asr. Choosing location of zero of the gravitational
...
7. The commonly encountered expression mgh for the potential energy is actually an
...
(d) The formula -GMm(1/r2
1/r₁) is more/less accurate than the formula

mg(r2 - r₁) for the difference of potential energy between two points r₂ and r₁ distance
...
7.5 Let us assume that our galaxy consists of 2.5 × 10¹¹ stars each of one solar mass. How
...
7.17 A rocket is fired vertically with a speed of 5 km s¯¹ from the earth's surface. How far
from the earth does the rocket go before returning to the earth? Mass of the earth
= 6.0 x 1024 kg; mean radius of the earth = 6.4 × 100 m; G = 6.67 x 10-11 N m² kg-2.

7.18 The escape speed of a projectile on the earth's surface is 11.2 km s¯¹. A body is
...
the earth = 6.4 × 106 m; G = 6.67 x 10-11 N m² kg-2.

7.20 Two stars each of one solar mass (= 2x1030 kg) are approaching each other for a
...
the line joining the centres of the spheres? Is an at that point in
equilibrium? If so, is the equilibrium stable or ?

# NCERT: 11TH_PHYSICS_CH08_MECHANICAL_PROPERTIES_OF_SOLIDS_CH08_MECHANICAL_PROPERTIES_OF_SOLIDS_EN
...
is known as stress. If Fis the force applied normal
to the cross-section and A is the area of cross
section of the body,

Magnitude of the stress = F/A
(8.1)
The SI unit of stress is Nm¯² or pascal (Pa)
...
displacement of the faces Ax to the length of the
cylinder L.
...
cylinder). Usually is very small, tan 0
is nearly equal to angle 0, (if 0 = 10°, for
example, there is only 1% difference between 0
and tan 0).
...
Fig. 8.1 (a) A cylindrical body under tensile stress elongates by AL (b) Shearing stress on a cylinder deforming it by
...
ultimate tensile strength (σ₁) of the material.
...
stress (σ) to the longitudinal strain (s) is defined as
...
stress i.e., N m¯² or Pascal (Pa). Table 8.1 gives
...
the length of a thin steel wire of 0.1 cm² cross-
...
clamp at one end, and the force Fis applied at
...
AL/AL = (Y/Y) × (L/L)
Given L = 2.2 m, L = 1.6 m,

S

From Table 9.1 Y = 1.1 × 1011 N.m-2, and
Y = 2.0 × 1011 N.m-2.
AL/AL = (2.0 × 1011/1.1 × 1011) × (2.2/1.6) = 2.5.
...
= π (1.5 × 10-3)2 × [(5.0 × 10-4 × 1.1 × 10¹¹)/2.2]
= 1.8 × 102 N
...
Table 8.2 Shear moduli (G) of some common
...
[(1078 × 0.5)/(9.4 × 10º × 1.26 x
= 4.55 x 10-5 m or 4.55 × 10-3 cm.
...
Example 8.4 A square lead slab of side 50
...
The shearing stress o can also be expressed as
...
occurs. That is, if p is positive, AV is negative.
...
Fractional compression AV/V, is
...
deforming force Falong the length of the wire,
let the length of the wire be elongated by I. Then
from Eq. (8.8), we have F = YA × (I/L). Here Yis
the Young's modulus of the material of the wire.
Now for a further elongation of infinitesimal
small length dl, work doned Wis F dl or YAldI/
L. Therefore, the amount of work done (W) in
increasing the length of the wire from L to L + 1,
that is from 1 = 0 to 1 = 1 is
...
= -x stress × strain x volume of the

wire

in the wire in the form of
...
EEEEEEE elas This work is stored

The strain perpendicular to the appli
...
is Ad, the lateral strain is Ad/d. If the original
length of the wire is Land the elongation under
stress is AL, the longitudinal strain is AL/L.
Poisson's ratio is then (Ad/d)/(∆L/L) or (Ad/AL)
...
yield strength (σ₁) of about 300 × 106 N m¯². Thus,
...
Fig. 8.6. A bar of length 1, breadth b, and depth
d when loaded at the centre by a load Wsags by
an amount given by
...
bis more effective in reducing the bending, since
8 is proportional to d¯³ and only to b¯¹(of course
the length I of the span should be as small as
...
Fig. 8.6 A beam supported at the ends and loaded
...
material of the mountain and gis the acceleration
...
There is a shear component, approximately hog
itself. Now the elastic limit for a typical rock is
30 × 107 N m-2. Equating this to hpg, with
p = 3 × 103 kg m-³ gives
...
applied force causing the strain, A is the cross-sectional area over which Fis applied
...
where p is the pressure (hydraulic stress) on the object due to the fluid, AV/V (the
...
the weight. However, the tension at any cross-section A of the wire is just Fand not 2F.
...
8.1 A steel wire of length 4.7 m and cross-sectional area 3.0 × 10-5 m² stretches by the same
amount as a copper wire of length 3.5 m and cross-sectional area of 4.0 × 10-5 m² under
...
8.4 Read the following two statements below carefully and state, with
...
stress is not to exceed 108 N m¯², what is the maximum load the cable can support ?

# NCERT: 11TH_PHYSICS_CH05_WORK_ENERGY_AND_POWER_CH05_WORK_ENERGY_AND_POWER_EN
...
Geometrically, B cos e is the projection of B onto
A in Fig.5.1 (b) and A cos e is the projection of A
...
The proofs of the above equations are
...
(5i+4j+3k) unit. Also find the

projection of F F on on d. d.
...
### 5.2 ΝΟΤIONS OF WORK AND KINETIC
...
where u and u are the initial and final speeds
...
(5.3)
where K and K are respectively the initial and
...
Assuming that gis a constant with a value
10 m/s², the work done by the gravitational force
...
where W is the work done by the resistive force
...
perpendicular. This is so since, for 0 = π/2
rad (= 90°), cos (π/2) = 0. For the block
moving on a smooth horizontal table, the
gravitational force mg does no work since
it acts at right angles to the displacement.
If we assume that the moon's orbit around
the earth is perfectly circular then the
earth's gravitational force does no work.
The moon's instantaneous displacement is
tangential while the earth's force is
radially inwards and 0 = π/2.

Work can be both positive and negative. If 0 is
between 0° and 90°, cos e in Eq. (5.4) is positive.
If @ is between 90° and 180°,cose is negative.
In many examples the frictional force opposes
displacement and 0 = 180°. Then the work done
by friction is negative (cos 180° = −1).
...
an angle of 180° (π rad) with each other.
...
with speed 200 m s¯¹ (see Table 5.2) on soft
...
is mv²/2 = 1000 J. It has a final kinetic energy
of 0.1×1000 = 100 J. If u is the emergent speed
of the bullet,
...
If the displacement Ax is small, we can take
...
Ax tends to zero. Thus, for a varying force
...
(b)
Fig. 5.3 (a) The shaded rectangle represents the
work done by the varying force F(x), over
the small displacement Ax, AW = F(x) ∆х.
(b) adding the areas of all the rectangles we
find that for Ax→0, the area under the curve
...
5.4. At x = 20 m, F = 50 N (≠ 0). We are given
that the frictional force fis | f | = 50 N. It opposes
...
W→ area of the rectangle ABCD + area of
...
Integrating from the initial position (x₁) to final
...
where, K and Kare the initial and final kinetic
...
force Fon the block in this range is inversely
...
energy and speed v of the block as it
crosses this patch?
...
The gravitational force on a ball of mass mis
mg.gmay be treated as a constant near the earth
surface. By 'near' we imply that the height h of
the ball above the earth's surface is very small
compared to the earth's radius R』(h <<R) so that
we can ignore the variation of gnear the earth's
...
the gravitational force Fequals the negative of
...
the base 10 [lnX = log X = 2.303 log10 X].
...
object of mass mis released from rest, from the
top of a smooth (frictionless) inclined plane of
heighth, its speed at the

bottom

is2gh irrespective of the angle of inclination.
...
AV is equal to the negative of the work done by
the force
...
Over the whole path, xto x, this means that
...
The total mechanical energies E, E, and EH
...
It is partially converted to kinetic at heighth and
is fully kinetic at ground level. This illustrates
the conservation of mechanical energy.

Example 5.7 A bob of mass mis suspended
by a light string of length L. It is imparted a
horizontal velocity v at the lowest point A
...
ve is the speed at C. From Eqs. (5.13)
...
the result from (i), namely v = 5gL,
...
spring, the spring force F is proportional to
...
unit is N m¯¹. The spring is said to be stiff if kis
large and soft if k is small.

Suppose that we pull the block outwards as in
Fig. 5.7(b). If the extension is x, the work done by
the spring force is
...
Fig. 5.7(d). Note that the work done by the
...
external force F does work + kx / 2. If the block
...
spring force. If the block of mass min Fig. 5.7 is
extended to x and released from rest, then its
total mechanical energy at any arbitrary point x,
where x lies between -x and +xm will be given by
...
constant 5.25 × 103 N m¯¹. What is the
...
where we have converted 18 km h¯¹ to 5 m s¯¹ [It is
useful to remember that 36 km h-1 = 10 m s¯¹].
At maximum compression x, the potential
...
ero is best defined at an infinite distance
...
0.5 x 103 x 10 = 5 × 103 N (taking
=10.0_ms¯²). After rearranging the above
equation we obtain the following quadratic
equation in the unknown Xm
k x + 2μm 2µm g xm – m v²
...
force Fc, the conservation of mechanical energy
...
unlike the conservative force, W depends on
...
quantity. Its dimensions are [ML2T-³]. In the SI,
its unit is called a watt (W). The watt is 1 J s¯¹.
...
Consider two masses m₁ and m₂. The particle
...
Fig. 5.10 Collisionofmassm₁,witha stationary massm2.
...
the collision time At cause a change in their
respective momenta :
...
collision time At. Since the third law is true at
...
collision time At is not constant. Such a collision
...
m₁v₁₁ = (m₁+m₂)v(momentum conservation)
...
above nomenclature with 0₁ = 0₂ = 0, the
...
m₁V1(U2f - U₁₁) = m₁V1 f (V2.f - U1f)
U2 f (Uri – V1 f) = vi – vis
...
Thus, the 'unknowns' {V₁ v2} are obtained in
terms of the 'knowns' {m₁, m₂, v₁₁}. Special cases
...
Case II : If one mass dominates, e.g. m₂ > > m₁
Vif - Vii
Vaf0
...
speed (typically 107 m s¯¹) must be slowed
to 103 m s-¹ so that it can have a high

probability of interacting with isotope 235U
...
heavy water (D₂O) or graphite, is called a
...
mass m₁ with the stationary mass m₂. Linear
...
velocity directions of m₁ and m₂ and choose it to
...
m₁v₁₁ = m vcos 0₁ + m2v2fcos 02 (5.28)
0 = m₁v₁ sin 0₁ – m2v2rsin θ₂
...
are thus four unknowns {v₁₁, V2f, 01 and 02}, and
only two equations. If 0₁ = 0 2 = 0, we regain
...
the four unknowns, say 0₁, must be made known
...
{m1, m2, v₁₁, θ₁} we can determine {V₁f, V2f, 02}
from Eqs. (5.28)-(5.30).
...
f = 1/9 while f₂ = 8/9. Almost 90% of the
neutron's energy is transferred to deuterium. For
carbon f₁ = 71.6% and f₁ = 28.4%. In practice,
...
billiard balls with equal masses m₁ = m2.
...
angle 02 = 37. Assume that the collision is
elastic and that friction and rotational

motion are not important. Obtain 01.
...
= { Uif V₁f² + U2f² + 2V1fU2f cos

(0₁ +37°)} (5.31)

Since the collision is elastic and m₁ = m, it follows
...
depends only on the end points {x, x}, or (ii) the work done by the force is zero for an
...
5. The gravitational potential energy of a particle of mass mat a height x about the earth's
...
quantity given by :A·B = AB cos 0, where @ is the angle between A and B. It can be
positive, negative or zero depending upon the value of 0. The scalar product of two
...
ii = j j = k + k = 1 and i j = j k = k • i = 0
...
### 3. For two bodies, the sum of the mutual forces exerted 
Newton's Third Law,

F12 + F21 = 0

seen them is zero from
...
potential energy kx²/2, the zero of the potential energy is the equilibrium position
...
in general. Yet the work done by the gravitational force over every
...
Its displacement in time tis proportional to
(i) t1/2
(ii) t
(iii) t3/2
...
entire journey if its speed on reaching the ground is 10 m s¯¹ ?

### 5.14 A molecule in a gas container hits a horizontal wall with speed 200 m s¯¹ and angle 30°
...
### 5.15 A pump on the ground floor of a building can pump up water to fill a tank of volume 30 m³
...
0.05 kg s¯¹. What is the speed of the trolley after the entire sand bag is empty ?

### 5.20 A body of mass 0.5 kg travels in a straight line with velocity v=ax3/2 where a = 5 m-1/2 s-1.
...
converts 25% of the wind's energy into electrical energy, and that A = 30 m², v = 36
km/h and the density of air is 1.2 kg m³. What is the electrical power produced ?

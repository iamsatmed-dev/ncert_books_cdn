# NCERT: 11TH_PHYSICS_CH03_MOTION_IN_A_PLANE_CH03_MOTION_IN_A_PLANE_EN
...
and its density is 2.7×103 kg m¯³ (a scalar).
...
over a letter, say u. Thus, both v and u
...
motion from point P (at time 1) to point P' (at time t').
...
tips S and Palso coincide, the two vectors are
...
the factor A but the direction is the same as that
of A:
...
product of the dimensions of A and A. For
...
vector A to get R₂ = (A – B). The vector R₁ = A + B
...
a speed of 35 m s¯¹. Winds starts blowing
after sometime with a speed of 12 m s¯¹ in
...
see that the resultant of v and vis Ras shown
...
Fig. 3.8 (a) Two non-colinear vectors a and b.
...
say ń by a scalar, the result is a vector
λ = λή. In general, a vector A can be written as
...
axes as in Fig. 3.9(b), and get vectors A₁ and A

such that A₁ + A₂ =

Ул

and A, is parallel to j, we have :
A₁= Ai, A₂ = Aj
...
This is represented in Fig. 3.9(c). The quantities
A and A are called x-, and y- components of the
vector A. Note that A is itself not a vector, but

y

A i is a vector, and so is A j. Using simple
trigonometry, we can express A and A in terms
of the magnitude of A and the angle e it makes
with the x-axis :
...
(i) its magnitude A and the direction 0 it makes
...
If A and are given, A and A can be obtained
using Eq. (3.13). If A and A₁₄ are given, A and 0
...
dimensions. If α, β, and y are the angles*
...
In A OSN, SN = OS sina = Rsina, and

in & PSN, SN = PS sin 0 = B sin 0

Therefore, Rsina = B sin 0
...
by the thick line and is at Pat time t and P' at
time ť [Fig. 3.12(b)]. Then, the displacement is :
(3.25)
...
Fig. 3.12 (a) Position vectorr. (b) Displacement Ar and
...
times At₁,At₂, and At3. Ar₁, Ar₂, and Ar3 are the
...
displacements of the object in times At₁, At₂, and
...
velocity is shown in figures (a), (b) and (c) for
three decreasing values of ∆t, i.e. st₁,∆t, and At3,
(st₁ > st₂ > ∆t). As at → 0, r → 0
...
and the direction of v is given by the angle 0:
...
v, v, and angle 0 are shown in Fig. 3.14 for a
...
time t and P₁, P2, P3 positions after time At₁, At₂,
At, respectively (At₁> At¸>∆t). The velocity vectors
at points P, P1, P2, P3 are also shown in Figs. 3.15
...
Fig. 3.15 The average acceleration for three time intervals (a) At₁, (b) At₂, and (c) At3, (At1> ∆t2> ∆t3). (d) In the
limit At→0, the average acceleration becomes the acceleration.
...
Example 3.4 The position of a particle is
given by

r = 3.0t i + 2.0t2j+5.0ќ
...
At_t = 1.0 s, v = 3.0i + 4.0ĵ

It's magnitude is v = √3² + 42 = 5.0 m s¯¹

and direction is

### 3.8 ΜΟΤION IN A PLANE WITH CONSTANT
ACCELERATION
...
of the object be vo at time t = 0 and v at time t.
Then, by definition
...
velocities at these instants be v and v. Then,
...
dr
gives Eq.(3.33a) and it also
dt
satisfies the condition that at_t=0, r = r.
...
One immediate interpretation of Eq.(3.34b) is that
...
situations, as we shall see in section 3.9 for
projectile motion.
...
Now, the velocity v = dt = (5.0+ 3.0t)i+2.0t j

At_t = 6 s, v = 23.0î+12.0ĵ

speed = v = √23² + 122 = 26 m s¯¹
...
Now, since g, 0 and v are constants, Eq. (3.39)
...
maximum height? Let this time be denoted by t
Since at this point, v=0, we have from Eq. (3.38):
...
We note that T = 2 tm, which is expected
...
range, R. It is the distance travelled during the time

of flight T. Therefore, the range Ris
...
The stone hits the ground when y(t) =
...
Equation (3.42a) shows that for a given

projection velocity v, Ris maximum when sin

20 is maximum, i.e., when 0 = 45°.
...
Example 3.8 A cricket ball is thrown at a
...
Now, for angles, (45 + a) and ( 45 − α), 20 is
(90 + 2a) and (90 2α), respectively. The
values of sin (90 + 2a) and sin (90 – 2α) are
the same, equal to that of cos 2a. Therefore,
ranges are equal for elevations which exceed or
fall short of 45 by equal amounts a.

Example 3.7 A hiker stands on the edge
of a cliff 490 m above the ground and
throws a stone horizontally with an initial
speed of 15 m s¯¹. Neglecting air resistance,
...
Fig. 3.18 Velocity and acceleration of an object in uniform circular motion. The time interval At decreases from
...
and P' as shown in Fig. 3.18(a). By definition,
velocity at a point is along the tangent at that
point in the direction of motion. The velocity
vectors v and v' are as shown in Fig. 3.18(a1).
Av is obtained in Fig. 3.18 (a2) using the triangle
...
average acceleration is along Av ā
...
r' be Ae. Since the velocity vectors v and v' are
...
v, v' and Av are similar (Fig. 3.18a). Therefore,
...
will also be small and then arc
...
Therefore, the centripetal acceleration a is :

* In the limit At→0, Ar becomes perpendicular to r. In this limit ∆v→0 and is consequently also perpendicular
...
speed v in a circle of radius Rhas a magnitude
v/Rand is always directed towards the centre.
...
'centre-seeking'. Since vand Rare constant, the
...
(Fig. 3.18) turns through an angle Ae as shown
in the figure. Aθ is called angular distance. We
...
frequency v (=1/T). However, during this time
the distance moved by the object is s = 2nR.
Therefore, v = 2πR/T=2πRv
...
motion. Here R= 12 cm. The angular speed wis

2π/Τ = 2π 7/100

= 0.44 rad/s

v = R = 0.44 s¯¹ 12 cm = 5.3 cm s¯¹
...
A vector A multiplied by a real number is also a vector, whose magnitude is a times
...
where A, A are its components along x-, and y -axes. If vector A makes an angle 0
...
0, A=A sin and A = |A| = √A + A, tand =

with the x-axis, then A₁ = A cos 0, A₁=A s
...
R = Rxi + Ruj, where, R = A + B₁₂, and R = A + B

x

y

12. The position vector of an object in x-y plane is given by r xi + yj and the
...
13. If an object undergoes a displacement Ar in time At, its average velocity is given by
...
14. If the velocity of an object changes from v to v'in time At, then its average acceleration
...
The acceleration a at any time t is the limiting value of a as At→0 :
...
In component form, we have : a = axi + a₁
dv.
duy
where, ax =
, ay =
...
15. If an object is moving in a plane with constant acceleration a = |a| = √a + aand
...
projected with initial velocity v making an angle 0 with x-axis and if we assume its
...
called uniform circular motion. The magnitude of its acceleration is a = v² /R. The
direction of a is always towards the centre of the circle.

The angular speed w, is the rate of change of angular distance. It is related to velocity
vby v = ω R. The acceleration is a = 2R.

If T is the time period of revolution of the object in circular motion and v is its
frequency, we have ω = 2π ν, υ = 2πνR, a = 4π²ν²R
...
### 5. An object subjected to two velocities v₁ and v₂ has a resultant velocity v = v₁ + v2. Take
...
velocity). For example, the trajectory of an object moving under the sa same acceleration
due to gravity can be a straight line or a parabola depending on the the initial conditions.
...
### 3.10 On an open ground, a motorist follows a track that turns to his left by an angle of 60°
...
a ball thrown with a speed of 40 m s¹ can go without hitting the ceiling of the hall ?
...
### 3.18 A particle starts from the origin at t = 0 s with a velocity of 10.0 ĵ m/s and moves in
the x-y plane with a constant acceleration of (8.0i+2.0j) m s². (a) At what time is
...
### 3.19 and j are unit vectors along x- and y- axis respectively. What is the magnitude
and direction of the vectors_i+j, and i-j? What are the components of a vector
A= 2 21+3) along the directions of i+ j and î - j? [You m

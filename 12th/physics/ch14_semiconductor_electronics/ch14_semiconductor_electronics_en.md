# NCERT: 12TH_PHYSICS_CH14_SEMICONDUCTOR_ELECTRONICS_CH14_SEMICONDUCTOR_ELECTRONICS_EN
...
(p = 1/6), the solids are broadly classified as:
...
The values of p and a given above are indicative of magnitude and
...
Natoms. For Si, the outermost orbit is the third orbit (n = 3), while for Ge
...
is shown as Ev. Above E and below Ev
...
Case II: In this case, as shown in Fig. 14.2(b), a large band gap E exists
(E > 3 eV). There are no electrons in the conduction band, and therefore
...
band gap (E < 3 eV) exists. Because of the small band gap, at room
...
In intrinsic semiconductors, the number of free electrons, nis equal to
the number of holes, n₁. That is

n = n = n₁

where n₁ is called intrinsic carrier concentration.
...
giving the hole current, I₁. The total current, Iis
thus the sum of the electron current I and the
hole current In:

hat apart from the a
...
and empty circles (0) represent holes.
...
out an electron from these atoms (i.e., ionisation energy E) will be
...
of
of 
an atom in the crystal lattice of Si, four of
...
n is due to the electrons contributed by donors and those generated
intrinsically, while the total number of holes n₁ is only due to the holes
...
the number (n)of intrinsically generated electrons to n.
...
impurities (E) and acceptor impurities (E₁) also exist. In the energy band
diagram of n-type Si semiconductor, the donor energy level E is slightly
below the bottom E of the conduction band and electrons from this level
...
for p-type semiconductor, the acceptor energy level E₁ is slightly above
the top E of the valence band as shown in Fig. 14.9(b). With very small
supply of energy an electron from the valence band can jump to the level
E and ionise the acceptor negatively. (Alternately, we can also say that
with very small supply of energy the hole from level E₁ sinks down into
...
one thermally generated electron-hole
pair + 9 electrons from donor atoms

(b) T > OK

FIGURE 14.9 Energy bands of (a) n-type semiconductor at T > OK, (b) p-type
semiconductor at T > OK.

EXAMPLE 14.2

Example 14.2 Suppose a pure Si crystal has 5 × 1028 atoms m¯³. It is
doped by 1 ppm concentration of pentavalent As. Calculate the
number of electrons and holes. Given that n₁ =1.5 × 1016 m¯³.

-3

Solution Note that thermally generated electrons (n₁ ~1016 m³) are
negligibly small as compared to those produced by doping.
Therefore, n≈ ND

2

Since nn = n², The number of holes
...
by applying an external voltage Vacross the diode. The
...
built-in potential V. As a result, the depletion layer width
decreases and the barrier height is reduced [Fig. 14.13(b)]. The
effective barrier height under forward bias is (V – V).
...
The effective barrier height under reverse bias is (V + V), [Fig. 14.15(b)].
...
breakdown voltage (V₁₄). When V = V♭, the diode reverse current
...
rfb = ∆V/ΔΙ = 0.1V/10 mA = 10 Ω

(b) From the curve at V = −10 V, I = −1 μΑ,
...
the Fig. 14.17. Calculate the resistance of the diode at (a) Ip
and (b) V = −10 V.
...
the ratio of small change in voltage AV to a small change in current ∆Ι:
...
is a current through the load resistor R₁ and we
...
D₁ gets forward biased and conducts (while
D₂ being reverse biased is not conducting).
Hence, during this positive half cycle we get
an output current (and a output voltage
across the load resistor R₁) as shown in
...
D₁ would not conduct but diode D₂ would,
giving an output current and output
voltage (across R₁) during the negative half
...
terminals (parallel to the load R₁). One can
also use an inductor in series with R₁ for
...
diode D₁ at A and to the diode D₂ at B;
(c) Output waveform across the
load R₁ connected in the full-wave
...
### 3. Metals have low resistivity (10¯² to 10¯° Ωm), insulators have very high
resistivity (>10º 2m¯¹), while semiconductors have intermediate values
of resistivity.
...
number of electrons (n) is equal to the number of holes (n₁) in intrinsic
...
7. In n-type semiconductors, n>> n₁ while in p-type semiconductors n₁>> n
...
n = n² in all cases. Further, the material possesses an overall charge
neutrality.
...
11. For insulators Eg > 3 eV, for semiconductors Eis 0.2 eV to 3 eV, while
for metals E ≈ 0.
...
to one direction). With the help of a capacitor or a suitable filter, a de
...
### 1. The energy bands (Ec or E₁) in the semiconductors are space delocalised
...
which Ec or E are drawn as straight lines, then they should be
respectively taken simply as as the bottom of conduction band energy levels
and top of valence band energy levels.

### 2. In elemental semiconductors (Si or Ge), the the n-type or
...
by energy band gap respectively equal to (E), (Eg)si and (Eg)Ge. Which
...
Prefix Symbol Factor Prefix symbol
...
v = 6.7cm. Magnification = 5/9, i.e., the size of the image
...
### 1.32; ngw 1.144; which gives sin r = 0.6181 i.e.,

r~ 38°.

9.5

r = 0.8 × tan iand sini = 1/1.33 = 0.75, where r is the radius (in m)

of the largest circle from which light comes out and i is the critical
angle for water-air interface, Area = 2.6m²
...
n = 1.53 and Dm for prism in water
...
(a) f = +20cm. Image is real and at 7.5 cm from the lens on its right
...
1.8cm. As u → ∞, v → f (but never beyond f while m → 0).
...
9.11 (a) v = -25cm and f = 6.25cm give u = −5cm; v = (15 – 5) cm =

10 cm,

fo = u = -2.5cm; Magnifying power = 20
...
(a) sini = 1.44/1.68 which gives i = 59°. Total internal reflection
takes place _when_i > 59° or when r < Imax = 31°. Now,
(sini /sinrmax) = 1.68, which gives imax
...
(b) If there is no outer coating, i
...
36.5°. Now,
i = 90° will have r = 36.5°and_i' = 53.5° which is greater than
i. Thus, all incident rays (in the range 53.5° < i < 90°) will
suffer total internal reflections.

9.18 For fixed distances between object and screen, the lens equation
...
f₁ = 30 cm and u₁ = ∞, give v₁ = + 30cm. This image becomes
a virtual object for the second lens.

f2 = −20 cm, u2 = + (30 – 8) cm = + 22 cm which gives,
U2 = 220 cm. The parallel incident beam appears to diverge
from a point 216cm from the centre of the two-lens system.

=

(ii) Let the parallel beam be incident from the left on the concave
lens first: f₁ = − 20 cm, u₁ = − ∞, give v₁ = – 20 cm. This image
becomes a real object for the second lens: f₂ = + 30 cm, u₂
(20 + 8) cm = – 28cm which gives, v₂ = 420 cm. The parallel
...
critical angle i, the angle of refraction or at the first face is (60°-i).
Now, i = sin¯¹ (1/1.524) ~ 41°
...
Each square in the virtual image has an area 10 ×
= 100 mm² = 1 cm²
...
(a) Maximum magnifying power is obtained when the image is at
...
(d) Angular magnification of eye-piece is [(25/f) + 1] (f in cm) which
increases if f is smaller. Further, magnification of the objective
...
which is large when |uo| is slightly greater than f. The micro-
scope is used for viewing very close object. So |uo| is small, and
so is fo
...
which gives u= −1.5cm; v= 7.5 cm. |u|= (25/6) cm = 4.17 cm. The
...
10.1 (a) Reflected light: (wavelength, frequency, speed sa
...
v = 5.09 × 1014 Hz, c = 3.00 × 108 ms¯¹

Refracted light: (frequency same as the incident frequency)
v = 5.09 × 10¹4Hz

v = (c/n) = 2.26 × 10º ms¯¹, λ = (v/v) = 444 nm
...
colour deviates more than red in a glass prism, i.e. n > n.
...
(a) 3.14 × 10-19J, 1.05 × 10-27 kg m/s
...
12.5 9.7 × 10¯°m; 3.1 × 1015Hz.

12.6 (a) 2.18 × 10°m/s; 1.09 × 10°m/s; 7.27 × 105m/s

12.7

12.8

(b) 1.52 × 10-16 s; 1.22 × 10~15s; 4.11 × 10-15 s.
2.12×10¯¹º m; 4.77 × 10-10 m
...
Q = m(Fe)-2m(Al) = 26.90 MeV; not possible.

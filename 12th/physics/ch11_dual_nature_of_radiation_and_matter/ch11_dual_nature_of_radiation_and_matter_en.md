# NCERT: 12TH_PHYSICS_CH11_DUAL_NATURE_OF_RADIATION_AND_MATTER_CH11_DUAL_NATURE_OF_RADIATION_AND_MATTER_EN
...
to 0.2 times the speed of light (3×10m/s). The presently accepted value
...
found to emit negatively charged particles. The value of e/mof these particles
...
The work function (6) depends on the properties of the metal and the
...
108 V m¯¹) to a metal, electrons can be pulled out of the metal, as in a
spark plug.
...
is measured by a microammeter (µA). The photoelectric
...
can be varied, as can the potential difference Vbetween
...
plate C and illuminate the plate C with light of fixed frequency v and fixed
intensity I₁. We next vary the positive potential of plate A gradually and
...
critical value of the negative potential Vo
on the plate A. For a particular frequency
of incident radiation, the minimum
negative (retarding) potential Vo given to
...
frequency but of higher intensity I2 and I3 (I3 > I2 > I₁). We note that the
...
same as that for the incident radiation of intensity I₁, as shown graphically
...
v of the incident radiation and the stopping
potential Vo. We suitably adjust the same
...
frequencies are in the order v3 > V2 > V1 . This
...
frequency vo for which the stopping potential
...
with frequency v of incident radiation for a
...
frequency vo, no photoelectric emission is possible even if the
intensity is large.

This minimum, cut-off frequency vo, is called the threshold frequency.
...
a time of the order of 10¯º s or less.
...
apparent time lag (~10¯ºs or less), even when the incident radiation is
...
has energy hv, where his Planck's constant and v the frequency of light.
In photoelectric effect, an electron absorbs a quantum of energy (hv) of
...
function ), the electron is emitted with maximum kinetic energy
...
According to Eq. (11.2), Kmax depends linearly on v,
...
function do, the higher the minimum or threshold
frequency vo needed to emit photoelectrons. Thus,
there exists a threshold frequency vo (= φ‰/h) for the
...
the metal (for v > vo). This explains why, for v > vo,
...
This is an important result. It predicts that the Vo
versus v curve is a straight line with slope = (h/e),
...
value was close to the value of Planck's contant (= 6.626 × 10~34Js)
...
of light quanta and the experimental determination of values of hand φο,
...
associated with momentum (h v/c). A definite value of energy as well as
...
(iii) All photons of light of a particular frequency v, or wavelength λ, have
the same energy_E (=hv = hc/^) and momentum p (= hv/c= h/λ),
...
radiation must be equal to work function Φο, so that
...
Thus Thus, for frequencies less than this threshold frequency, no
photoelectrons are ejected.

(b) Photocurrent reduces to zero, when maximum kinetic energy of
the emitted photoelectrons equals the potential energy e V by the
retarding potential V. Einstein's Photoelectric equation is
...
length associated with a particle of momentum pis
given as
...
wherem is the mass of the particle and v its speed.
...
the wavelength of the matter wave is called de Broglie wavelength. The
dual aspect of matter is evident in the de Broglie relation. On the left hand
side of Eq. (11.5), λ is the attribute of a wave while on the right hand side
...
of a ball of mass 0.12 kg moving with a speed of 20 m s¯¹ is easily
...
electron moving with a speed of 5.4×10º m/s, and (b) a ball of mass 150 g
...
the work function (6) required for electron emission from the metal
...
### 4. The stopping potential (V) depends on (i) the frequency of incident
...
### 5. Below a certain frequency (threshold frequency) võ, characteristic of
...
intensity, the existence of v and the instantaneous nature of the
...
energy E (= hv) and momentum p (= h/λ), which depend on the
frequency (v) of incident light and not on its intensity. Photoelectric
...
conservation law as applied to the photon absorption by an an electron in
the metal. The maximum kinetic energy (1/2)m v²nax is equal to
the photon energy (hv) minus the work function (= hv) of the
target metal:
...
9. The de Broglie wavelength (λ) associated with a moving particle is
related to its momentum pas: λ = h/p. The dualism of matter is
inherent in the de Broglie relation which contains a wave concept
(λ) and a particle concept (p). The de Broglie wavelength is
...
Physical Symbol Dimensions Unit
Quantity
...
significance; its phase velocity up has no physical significance. However,
...
frequency 6 ×10¹4Hz is incident on the metal surface, photoemission
...
(c) a dust particle of mass 1.0 × 10¯º kg drifting with a speed of 2.2
m/s?

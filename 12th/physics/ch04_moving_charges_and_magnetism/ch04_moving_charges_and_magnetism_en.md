# NCERT: 12TH_PHYSICS_CH04_MOVING_CHARGES_AND_MAGNETISM_CH04_MOVING_CHARGES_AND_MAGNETISM_EN
...
where ŕ is unit vector along r, and the field E is a vector
...
with a velocity v and, located at rat a given time t) in
...
equation F = q [ v × B] =qv B sin 0 ń, where dis
...
smaller unit (non-SI) called gauss (=104 tesla) is also often used. The
...
drift velocity v』 (see Chapter 3). In the presence of an external magnetic
field B, the force on these carriers is:

F = (nlA)q va × B

where q is the value of the charge on a carrier. Now nqv』 is the current
density jand | (nqv₁) | A is the current I (see Chapter 3 for the discussion
...
on it by considering it as a collection of linear strips dļ and summing
...
Solution
The velocity v of particle is along the x-axis, while B, the
...
then a force of mv² / r, acts perpendicular to the path towards
...
force is perpendicular to both v and B and acts like a
centripetal force. It has a magnitude q v B.
Equating the two expressions for centripetal
force,
...
described. If @ is the angular frequency, then v
= @ r. So,
...
Here v is the frequency of rotation. The
independence of v from energy has important
application in the design of a cyclotron.

The time taken for one revolution is T= 2π/
@ = 1/v. If there is a component of the velocity
parallel to the magnetic field (denoted by v₁₁),
it will make the particle move along the field and the path of the
...
particle would be a helical one (Fig. 4.6). The distance moved along
the magnetic field in one rotation is called pitch p. Using Eq. [4.6 (a)],
we have
...
The radius of the circular component of motion is called the radius
of the helix.
...
a magnetic field of 6 × 104 T perpendicular to it? What is its
...
r = mv / (qB) = 9 ×10-31 kg × 3 × 107m s¯¹ / ( 1.6 × 10~19 C × 6 × 10~4

= 28 × 10-2 m = 28 cm

v = v / (2 πι) = 17×106 s¯¹ = 17×10° Hz_=17 MHz.

E = (12)mv² = (³½ ) 9 × 10-31 kg × 9 × 1014 m²/s² 40.5 ×10-17 J
...
### 4.4 Magnetic FIELD DUE to a CuRRENT
...
I, the element length | d11, and inversely proportional to the square
...
[4.7(a)] distance r. The

where μο/4π is a constant of proportionality. The above expression field is perpendicular
holds when the medium is vacuum.

*

The sense of dlx r is also given by the Right Hand Screw rule: Look at the
...
We call µo the permeability of free space (or vacuum).
...
the magnetic field is linear in the source Idl just as the electrostatic
...
charge. The magnetic field is produced by a vector source Idl.
...
plane containing the displacement vector rand the current element Idl.
...
0 = 0, sin 0 = 0 and from Eq. [4.7(a)], |dB| = 0.

There is an interesting relation between ɛ, the permittivity of free
space; u, the permeability of free space; and c, the speed of light in vacuum:
1
...
the product με is fixed in magnitude. Choosing the value of either & or
μο, fixes the value of the other. In SI units, μ is fixed to be equal to
4 × 10-7 in magnitude.
...
(IdI) mentioned in the previous section. We assume that
...
Consider a conducting element d/ of the loop. This is
shown in Fig. 4.9. The magnitude dB of the magnetic
field due to d/ is given by the Biot-Savart law [Eq. 4.7(a)],
...
the element to the axial point. For example, the element d/ in Fig. 4.9 is

in the y-z plane, whereas, the displacement vector r from d/ to the axial
...
plane formed by dl and r. It has an x-component dB and a component
...
contribution due to the diametrically opposite di element, shown in
Fig. 4.9. Thus, only the x-component survives. The net contribution along
x-direction can be obtained by integrating dB = dB cos @over the loop.
For Fig. 4.9,
...
The summation of elements dlover the loop yields 2πR, the
...
of the loop. Here_x = 0, and we obtain,
...
Therefore, dl x r = 0. Straight segments do not contribute to
BI.
...
circular loop. Thus B is 1.9 × 104 T normal to the plane of the
paper going into it.
...
magnetic field, B₁, at this element and multiply it by the
...
length of that element dl [Note: B₁dl=B•d1]. All such
...
states that this integral is equal to times the total
...
Eq. [4.13(a)] proves sufficient. We shall assume that, in
such cases, it is possible to choose the loop (called
an amperian loop) such that at each point of the
loop, either

(i) Bis tangential to the loop and is a non-zero constant
B, or
...
Now, let Lbe the length (part) of the loop for which B
is tangential. Let I be the current enclosed by the loop.
...
side of Eq. [4.13 (b)], Β. 2πr. We find that the magnetic
...
I = Current enclosed by the loop = I
...
Now the current enclosed I is not I, but is less than this value.
Since the current distribution is uniform, the current enclosed is,

Using Ampere's law, B(2tr) = μο
...
B = μοI/2R [Eq. (4.12)] for the field at the centre of the loop. However,
...
the magnetic field produced by a commonly used and very useful
...
as argued above. Along transverse sections be and ad, the field component
...
of turns is nh. The enclosed current is, I = I (nh), where Iis the current
...
set up by conductor 'a' at conductor 'b'.
...
by a distance d and carrying (parallel) currents I and I, respectively.
The conductor 'a' produces, the same magnetic field B₂ at all points
...
The conductor ‘b' carrying a current I will experience a sideways
force due to the field B. The direction of this force is towards the
conductor 'a' (Verify this). We label this force as F, the force on a
...
length L of 'a' due to the current in 'b'. It is equal in magnitude to Fa
...
Let fba represent the magnitude magnitude of the force F Fba per unit length. Then,
...
This is larger than the value 2×10-7 Nm¯¹ quoted in the definition
...
and exerts a force F₁ on it which is directed into the
...
Similarly, it exerts a force F, on the arm CD and F,
...
torque on the loop due to the pair of forces F₁ and F2.
...
m points downwards. The torque tis
...
force or torque. The forces on arms AB and CD are F₁ and F2. They too
...
ABCD makes an arbitrary angle @ with
the magnetic field. (b) Top view of
the loop. The forces F₁ and F₂ acting
...
As 0→0, the perpendicular distance between
...
angle between m and B is 0, Eqs. (4.20) and (4.21)
...
magnetic moment are [A][L2] and its unit is Am².

From Eq. (4.23), we see that the torque τ
...
If the loop has Nclosely wound turns, the expression for torque, Eq.
...
m = NIA = ΝΙπr² = 100 × 3.2 × 3.14 × 10-2 = 10 A m²

The direction is once again given by the right-hand thumb rule.

(c) t = m × B [from Eq. (4.23)]

= m B sin θ

Initially, 0 = 0. Thus, initial torque τ₁ = 0. Finally, θ = π/2 (or 90°).

Thus, final torque t₁= m B = 10 × 2 = 20 N m.
...
(a) No, because that would require it to be in the vertical direction.
...
the loop. For x >> R, we may drop the R² term in the denominator. Thus,
...
Note that the area of the loop A = π尺². Thus,
...
where x is the distance from the dipole. If we replace p → mand µ →1/

in the above expression, we obtain the result for B for a point in the
plane of the loop at a distance x from the centre. For_x>>R,
...
the charges (or electric monopoles). In magnetism, a magnetic
...
the field is radial by design, we have taken sin 0 = 1 in
the above expression for the torque. The magnetic
torque NIAB tends to rotate the coil. A spring S
Permanent magnet provides a counter torque ko that balances the
magnetic torque NIAB; resulting in a steady angular
deflection 4. In equilibrium
...
where kis the torsional constant of the spring; i.e. the
restoring torque per unit twist. The deflection is
...
overcome these difficulties, one attaches a small resistance r, called shunt
...
If r has small value, in relation to the resistance of the rest of the

S

circuit R, the effect of introducing the measuring instrument is also small
...
galvanometer as the deflection per unit current. From Eq. (4.26) this
...
0.022 + 32 = 3.02 Ω . Hence, I = 3/3.02 = 0.99 A.
...
2. A straight conductor of length 1 and carrying a steady current I
experiences a force F in a uniform external magnetic field B,
F = 11 × B

where|1| = 1 and the direction of 1 is given by the direction of the
current.
...
element di carrying a steady current I at a point Pat a distance r from
...
where is the equilibrium deflection and k the torsion constant of
the spring.

11. A moving coil galvanometer can be converted into a ammeter by
introducing a shunt resistance r, of small value in parallel. It can be
...
sensitivity of M₂ and M₁.

10-4 T) is

In a chamber, a uniform magnetic field of 6.5 G (1 G = mag
maintained. An electron is shot into the field with a speed of
4.8 × 106 m s¯¹ normal to the field. Explain why the path of the
electron is a circle. Determine the radius of the circular orbit.
(e = 1.5 × 10-19 C, m = 9.1×10−31 kg)

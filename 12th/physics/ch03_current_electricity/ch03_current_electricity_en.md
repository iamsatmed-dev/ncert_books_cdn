# NCERT: 12TH_PHYSICS_CH03_CURRENT_ELECTRICITY_CH03_CURRENT_ELECTRICITY_EN
...
the current as follows. Let AQ be the net charge flowing across a cross-
section of a conductor during the time interval At [i.e., between times t
and (t + At)]. Then, the current at time t across the cross-section of the
conductor is defined as the value of the ratio of AQ to At in the limit of At
tending to zero,
...
and similarly – at the other disc. We attach
...
FIGURE 3.1 Charges + and put at the ends
...
and - are continuously replenished.
...
was discovered. Imagine a conductor through which a current Iis flowing
...
of the combination RĈ is [from Eq. (3.3)],
...
For a given voltage Vacross the slab, if I is the current
...
the resistance. In general, then the resistance Ris inversely proportional
...
where the constant of proportionality p depends on the material of the
conductor but not on its dimensions. p is called resistivity.
...
are A/m². Further, if E is the magnitude of uniform electric field in the
...
where σ=1/p is called the conductivity. Ohm's law is often stated in an
...
zero since their directions are random. Thus, if there are Nelectrons and
...
time more than tand some less than τ. In other words, the time t₁ in
...
N-electrons at any given time t gives us for the average velocity va
...
there are n ∆t |va|A_such electrons. Since each
electron carries a charge –e, the total charge transported across this area
A to the right in time ∆t is −ne A|va|∆t. E is directed towards the left and
hence the total charge transported along E across the area is negative of
this. The amount of charge crossing the area A in time At is by definition
...
At, all electrons to the left of the area at distances upto
|va|At would have crossed the area. If n is the number
...
The vector jis parallel to E and hence we can write Eq. (3.21) in the
vector form
...
electrons in a copper wire of cross-sectional area 1.0 × 107־ m² carrying
a current of 1.5 A. Assume that each copper atom contributes roughly
one conduction electron. The density of copper is 9.0 × 103 kg/m³,
...
Now, e = 1.6 × 10-19 C, A = 1.0 × 10~7m², I = 1.5 A. The density of
...
8.5 × 1028 x 1.6×10-19 ×1.0×10-7
= 1.1 × 103 m s¯¹ = 1.1 mm s¯¹

(b) (i) At a temperature T, the thermal speed* of a copper atom of
mass M is obtained from [<(1/2) Mư² > = (3/2) kT ] and is thus
...
an electromagnetic wave, namely equal to 3.0 × 10º m s-1
...
few mm s¯¹ for currents in the range of a few amperes? How then
...
An important quantity is the mobility u defined as the magnitude of
...
The SI unit of mobility is m²/Vs and is 10⁴ of the mobility in practical
units (cm²/Vs). Mobility is positive. From Eq. (3.17), we have
...
Metals have low resistivities in the range of 108 Ωm to 106 Ωm. At the
...
where pr is the resistivity at a temperature Tand
reference temperature To. a is called the temperature co-efficient of
...
p thus depends inversely both on the number n of free electrons per unit
...
causes p to increase as we have observed.

lished

For insulators and semiconductors, however, n increases with
temperature. This increase more than compensates any decrease in tin
Eq.(3.23) so that for such materials, p decreases with temperature.
...
Solution R = 5 Ω, R100 = 5.23 Ω and R₁ = 5.795 Ω
...
In a time interval ∆t, an amount of charge ∆Q = I At travels from A to
B. The potential energy of the charge at A, by definition, was Q V(A) and
similarly at B, it is Q V(B). Thus, change in its potential energy AU

AU = Final potential energy – Initial potential energy
...
as the power loss (“ohmic loss") in a conductor of resistance Rcarrying a
...
device R, to which a power Pis to be delivered via transmission cables
having a resistance R to be dissipated by it finally. If V is the voltage
across Rand I the current through it, then
...
connecting wires is inversely proportional to V². The transmission cables
from power stations are hundreds of miles long and their resistance Ris
...
potential – (V_ ) (V_ > 0) relative to the electrolyte adjacent to it,
...
between P and N is V₁ – (−V) = V₄ + V_ . This difference is called the
electromotive force (emf) of the cell and is denoted by ɛ. Thus
...
Note that & is, actually, a potential difference and not a force. The
...
To understand the significance of ɛ, consider a resistor R
...
Thus, emf & is the potential difference between the positive and
negative electrodes in an open circuit, i.e., when no current is

flowing through the cell.

If however R is finite, Iis not zero. In that case the potential difference
...
Note the negative sign in the expression (Ir) for the potential difference
...
may be neglected when the current I is such that ɛ >> I r. The actual
...
and it is Imax = ɛ/r. However, in most cells the maximum allowed current
...
and an internal resistance Leg
...
81, 82 are the emf's of the two cells and r₁, r₂ their internal resistances,
respectively.
...
Eeq and internal resistance req' r
...
Eq. (3.42) would change to VBC = −2−Ir2 and we will get
...
for &eq with a negative sign, as in Eq. (3.47).

Next, consider a parallel combination of the cells (Fig. 3.14).
I₁ and I, are the currents leaving the positive electrodes of the
...
Points B₁ and B₂ are connected exactly similarly to the second
...
B2, of emf &eq and internal resistance req, we would have
...
If there are n cells of emf &₁, &n and of internal
resistances r₁,... r respectively, connected in parallel, the
combination is equivalent to a single cell of emf &eq and

internal resistance r such that
...
Electric circuits generally

LES

number of

and cells interconnected sometimes in a
...
the cell. This will tell us the potential difference, V = V (P) – V (N) = ɛ-Ir
...
where R is the resistance of each edge and ɛ the emf of battery. Thus,
...
For R = 1 Ω, R = (5/6) Ω and for ɛ = 10 V, the total current (= 3I) in
the network is

31 = 10 V/(5/6) Ω = 12 A, i.e., I = 4A
...
We then have three unknowns I1, I2 and I which can be found by
...
10 – 4(I₁– I₂) + 2(I₂ + I3 − I₁) – I₁ = 0

that is, 711-612 – 213 = 10
...
current I I through G. Of special interest, is the case of a balanced bridge
...
and the second loop gives, upon using I₃ = 11, 14 = I2
...
A galvanometer of 152 resistance is connected across BD. Calculate
...
proportional to the voltage V across its ends, i.e., V∝ Ior V = RI,
...
### 4. The resistance R of a conductor depends on its length I and
...
where p, called resistivity is a property of the material and depends on
temperature and pressure.

### 5. Electrical resistivity of substances varies over a very wide range. Metals
have low resistivity, in the range of 108 Ωm to 106 Ωm. Insulators
...
(b) the relation between Vand I depends on the sign of V for the same

absolute value of V.

(c) The relation between Vand I is non-unique.
...
11. When a source of emf & is connected to an external resistance R, the
...
Equation E = pj leads to another statement of Ohm's law, i.e., a
...
va (Chapter 10, Textbook of Class XI). v₁, thus is only due to applied
electric field on the electron.

### 5. The relation j = p v should be applied to each type of charge carriers
...
Thus, the relation j = pv does not apply to the total current charge
density.
...
A silver wire has a resistance of 2.1 2 at 27.5 °C, and a resistance
of 2.7 2 at 100 °C. Determine the temperature coefficient of
...
A storage battery of emf 8.0 V and internal resistance 0.5 Ω is being
charged by a 120 V dc supply using a series resistor of 15.5 2. What
...
estimated in Example 3.1 is 8.5 × 1028 m¯³. How long does an electron
take to drift from one end of a wire 3.0 m long to its other end? The
area of cross-section of the wire is 2.0 × 10-6 m² and it is carrying a
current of 3.0 A.

# NCERT: 12TH_PHYSICS_CH10_WAVE_OPTICS_CH10_WAVE_OPTICS_EN
...
geometrical optics and a ray is defined as the path of of energy
...
FIGURE 10.2 F₁F₂ represents the spherical wavefront (with O as
centre) at t = 0. The envelope of the secondary wavelets

emanating from F₁F₂ produces the forward moving wavefront G₁G₁₂.

The backwave D₁D₂ does not exist.
...
draw spheres of radius υτ from each point on the spherical wavefront
where u represents the speed of the waves in the medium. If we now draw
a common tangent to all these spheres, we obtain the new position of the
wavefront at t = t. The new wavefront shown as G₁G₂ in Fig. 10.2 is again
spherical with point O as the centre.

1

The above model has one shortcoming: we also have a backwave which
is shown as D₁D₂ in Fig. 10.2. Huygens argued that the amplitude of the
...
corresponds to v₂ < v₁ so that the refracted waves bends towards the
...
wavefront, we draw a sphere of radius v₂t from the point
A in the second medium (the speed of the wave in the
second medium is v₂). Let CE represent a tangent plane
drawn from the point C on to the sphere. Then, AE = V₂ τ
...
where i and rare the angles of incidence and refraction,
...
(v₁). This prediction is opposite to the prediction from
...
This is the Snells law of refraction. Further, if λ₁ and 12 denote the
wavelengths of light in medium 1 and medium 2, respectively and if the
distance BC is equal to a ₁ then the distance AE will be equal to 12 (because
if the crest from B has reached C in time t, then the crest from A should
have also reached Ein time t); thus,
...
surface MN. If u represents the speed of the wave in the medium and if t
...
denser medium (v₁ > v₂) the wavelength and the speed of propagation
decrease but the frequency v (= v/λ) remains the same.

10.3.2 Refraction at a rarer medium

We now consider refraction of a plane wave at a rarer medium, i.e.,
v₂ > v₁. Proceeding in an exactly similar manner we can construct a
...
n₁ sin i = n₂ sin r . We define an angle i by the following equation
...
Thus, if i = i then sin r = 1 and r = 90°. Obviously, for i > i¸, there can
not be any refracted wave. The angle i is known as the critical angle and
...
Since the distances S₁ P and S₂ Pare equal, waves from S₁ and S₂ will
take the same time to travel to the point P and waves that emanate from
S₁ and S₂ in phase will also arrive, at the point P, in phase.

Thus, if the displacement produced by the source S₁ at the point Pis
given by
У1
...
then, the displacement produced by the source S, (at the point P) will S
...
where I represents the intensity produced by each one of the individual
...
The waves emanating from S₁ will arrive exactly two cycles earlier
than the waves from S₂ and will again be in phase [Fig. 10.9(a)]. Thus, if
the displacement produced by S₁ is given by
...
Y₂ = a cos (ωt −4π) = a cos ot

where we have used the fact that a path difference of 20 corresponds to a
phase difference of 4π. The two displacements are once again in phase
and the intensity will again be 4 I giving rise to constructive interference.
In the above analysis we have assumed that the distances S₁Q and S2Q
are much greater than d (which represents the distance between S₁ and
S₂) so that although S₁Q and S₂Q are not equal, the amplitudes of the
...
where we have used the fact that a path difference of 2.50 corresponds to
...
To summarise: If we have two coherent sources S₁ and S₂ vibrating
in phase, then for an arbitrary point P whenever the path difference,
S₁P ~ S2P = ηλ (n = 0, 1, 2, 3,...)

(10.9)

we will have constructive interference and the resultant intensity will be
41; the sign ~ between_S₁Pand S₂ P represents the difference between
S₁P and S₂ P. On the other hand, if the point P is such that the path
difference,

λ

S₁P ~ S₂P = (n+1) (n = 0, 1, 2, 3, ...)
...
difference between the two displacements be 4. Thus, if the displacement
produced by S₁ is given by
...
then, the displacement produced by S, W
...
The amplitude of the resultant displacement is 2a cos (6/2) and
...
If ¢ = 0, ± 2 π, ± 4 π,... which corresponds to the condition given by
Eq. (10.9) we will have constructive interference leading to maximum
intensity. On the other hand, if ¢ = ± π, ± 3π, ± 5π ... [which corresponds to
...
up and down regularly) then the phase difference é at any point will not
...
S₁ and S₂, the intensities will add
...
coming out from S₁ and S₂ are derived from the same original source
and any abrupt phase change in S will manifest in exactly similar phase
changes in the light coming out from S₁ and S2. Thus, the two sources S₁
and S₂ will be locked in phase; i.e., they will be coherent like the two
vibrating needle in our water wave example [Fig. 10.8(a)].

The spherical waves emanating from S₁ and S₂ will produce
...
source S₁ and S, on the screen GG' (Fig. 10.12); d = 0.025 mm, D = 5 cm
and λ = 5 × 10-5 cm.) (Adopted from OPTICS by A. Ghatak, Tata McGraw
...
treated as parallel, making an angle @ with the
normal MC.
...
maximum at 0 = 0 and other secondary maxima at 0≈
(n+1/2) λ/α, which go on becoming weaker and weaker
with increasing n. The minima (zero intensity) are at 0≈
ηλ/α, η = ±1, ±2, ±3, ....
...
The equipment needed can be found in most homes
two razor blades
...
of the displacement at x
in the +x-direction. At x = Ax, the time variation of the
displacement will be e slight slightly displaced to the right.

y (x,t) = a sin (kx – ot)

=

0 when a sinusoidal wave is propagating

(10.15)

where a and (= 2πν) represent the amplitude and the angular frequency
...
through a polaroid sheet P₁ it is observed that its intensity is reduced by
half. Rotating P₁ has no effect on the transmitted beam and transmitted
intensity remains constant. Now, let an identical piece of polaroid P₂ be
placed before P₁. As expected, the light from the lamp is reduced in
...
that light passing through the polaroid P₂ gets polarised along the pass-
axis of P2. If the pass-axis of P₂ makes an angle @ with the pass-axis of
P₁, then when the polarised beam passes through the polaroid P₂, the
component Ecos 0 (along the pass-axis of P₂) will pass through P2.
Thus, as we rotate the polaroid P₁ (or P₂), the intensity will vary as:
...
FIGURE 10.18 (a) Passage of light through two polaroids P₂ and P₁. The
...
P does not vary with angle. (b) Behaviour of the electric vector
...
Solution Let I be the intensity of polarised light after passing through
the first polariser P₁. Then the intensity of light after passing through
second polariser P₂ will be

I = Icos20,

where @ is the angle between pass axes of P₁ and P2. Since P₁ and P3
are crossed the angle between the pass axes of P₂ and P3 will be
(π/2-0). Hence the intensity of light emerging from P₂ will be
...
maximum. The intensity falls to zero at angles of ±2,±21 etc.,
...
glass? (Speed of light in vacuum is 3.0 × 10º m s¯¹)
...
path difference is 1, is Kunits. What is the intensity of light at a

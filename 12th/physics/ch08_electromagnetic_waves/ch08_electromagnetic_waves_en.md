# NCERT: 12TH_PHYSICS_CH08_ELECTROMAGNETIC_WAVES_CH08_ELECTROMAGNETIC_WAVES_EN
...
the speed of light( 3 ×10° m/s), obtained from optical
...
stretching from y rays (wavelength ~10-12 m) to long
radio waves (wavelength ~10º m) is described.
...
magnitude of the field, the left side of Eq. (8.1) is B (2πι).
So we have
...
Eq. (8.1) has not changed but the right hand side is zero and not µoi,
...
that one gets the same magnetic field at point P, no matter what surface i(t)
is used.
...
electric field E between the plates is (Q/A)/ɛ (see Eq. 2.41). The field is
...
is the electric flux 4』 through the surface S? Using Gauss's law, it is
...
the surface, another term which is ɛο times the rate of change of electric
...
obtained anywhere using the generalised Ampere's law. Bat the point P
...
denoted by i, and the displacement current denoted by ia (= &0
(do/dt)). So we have
...
In all respects, the displacement acement current has the same physical
...
Here kis related to the wave length

A of the wave by the usual
equation
...
and @ is the angular frequency. kis the magnitude of the wave vector (or
propagation vector) k and its direction describes the direction of
propagation of the wave. The speed of propagation of the wave is (@/k).
Using Eqs. [8.7(a) and (b)] for Ex and B and Maxwell's equations, one
...
in terms of frequency, v (=ω/2π) and wavelength, λ (=2π/k) as
...
medium are described in terms of a permittivity ɛ and a magnetic
permeability μ (these describe the factors by which the total fields differ
from the external fields). These replace ɛ and μ in the description to
electric and magnetic fields in Maxwell's equations with the result that in
a material medium of permittivity ɛ and magnetic permeability μ, the
...
of a value of 3×10° m/s. The constancy of the velocity of em waves in
...
point in space and time, E = 6.3 jV/m. What is B at this point?
...
be along x-direction. Since, (+ j) × (+ќ) = i, B is along the z-direction.
Thus,
...
given by B = (2 × 10-7) T sin (0.5×103x+1.5×10¹¹t).
...
(b) E = Boc = 2×10^7 T × 3 × 10° m/s = 6 × 10¹ V/m
...
covers wavelengths from about 10¯º m (10 nm) down to 10-13 m
(10¯⁴ nm). One common way to generate X-rays is to bombard a metal
...
oscillating harmonically with frequency v, produces electromagnetic waves
of the same frequency v. An electric dipole is a basic source of
...
the electromagnetic wave. For a wave of frequency v, wavelength λ,
...
They are related by E/Bo = c.

2π

### 5. The speed c of electromagnetic wave in vacuum is related to μ and ɛ (the
free space permeability and permittivity constants) as follows:

c = 1/√μο. The value of c equals the speed of light obtained from
...
where u is the permeability of the medium and & its permittivity.
...
names; y-rays, X-rays, ultraviolet rays, visible rays, infrared rays,
...
with a frequency of 10º Hz. What is the frequency of the
...
is E = 120 N/C and that its frequency is v = 50.0 MHz. (a) Determine,
Βο,ω, k, and λ. (b) Find expressions for E and B.

8.9

The terminology of different parts of the electromagnetic spectrum
is given in the text. Use the formula E = hv (for energy of a quantum

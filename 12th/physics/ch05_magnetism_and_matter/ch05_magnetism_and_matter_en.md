# NCERT: 12TH_PHYSICS_CH05_MAGNETISM_AND_MATTER_CH05_MAGNETISM_AND_MATTER_EN
...
nd is the angle by restablished
...
In magnitude τ = mB sine

Here

(5.2)

is restoring torque and @ is the angle between m and B.
An expression for magnetic potential energy can be obtained on lines

similar to electrostatic potential energy.

The magnetic potential energy U is given by
...
zero means fixing the zero of potential energy at 0 = 90°, i.e., when the
...
at a distance r, for r>> l, where lis the size of the magnet:
...
For instance for the configuration Q3 for which is along the
perpendicular bisector of the dipole P, the magnetic moment of is
...
byi, the number of lines leaving the surface is equal to
...
Gaussian surfaces represented byⓘor i in Fig 5.2(a) or
...
Fig. 5.5. The magnetic flux through ÄS is defined as AB = B.AS, where B
is the field at AS. We divide Sinto many small area elements and calculate
the individual flux through each. Then, the net flux op is,
...
where 'all' stands for 'all area elements AS'. Compare this with the Gauss's
...
M is a vector with dimensions L¯¹ A and is measured in a units of Am¯¹.
...
where µo is the same constant (permiability of vacuum) that appears in
Biot-Savart's law.
...
where H has the same dimensions as M and is measured in
...
where X, a dimensionless quantity, is appropriately called the magnetic
susceptibility. It is a measure of how a magnetic material responds to an
external field. X is small and positive for materials, which are called
...
where μ = 1 + x, is a dimensionless quantity called the relative magnetic
permeability of the substance. It is the analog of the dielectric constant in
electrostatics. The magnetic permeability of the substance is u and it has
the same dimensions and units as μο;

μ = μομ₁ = μο (1+x).

The three quantities χ, μ₁ and u are interrelated and only one of
...
H = nl = 1000 × 2.0 = 2 ×10³ A/m.

(b) The magnetic field B is given by

Β = με μο Η

= 400 × 4π × 107־ )N/A²) × 2 × 103 (A/m(
...
core. Thus B = μ₁n (I + I₁₁). Using I = 2A, B = 1 T, we get IM = 794 A.

The discussion in the previous section helps us to classify materials asd
...
x, a material is diamagnetic if x is negative, para- if x is positive and small,
and ferro- if x is large and positive.

A glance at Table 5.2 gives one a better feeling for these materials.
Here & is a small positive number introduced to quantify paramagnetic
...
expelled! x = −1 and µ₁ = 0. A superconductor repels a magnet and (by
...
oxygen (at STP) and copper chloride. For a paramagnetic material both x
and u depend not only on the material, but also (in a simple fashion) on
...
### 3. Consider a bar magnet of size 1 and magnetic moment m, at a distance
...
6. For a linear material M = x H. So that B = μ Η and x is called the
magnetic susceptibility of the material. The three quantities, x, the
relative magnetic permeability µ₁, and the magnetic permeability u are
...
and ferromagnetic. For diamagnetic materials x is negative and small
and for paramagnetic materials it is positive and small. Ferromagnetic
materials have large x.
...
electronic charge with value lel = 1.6 ×10-19 C. All other charges are
...
### 4. A miniscule difference in the value of x, the magnetic susceptibility, yields
radically different behaviour: diamagnetic versus paramagnetic. For
diamagnetic materials x = −10-5 whereas x = +10¯5 for paramagnetic
...
metal at very low temperatures. In this case x = −1, μ₁ = 0, μ = 0. The
...
BCS theory was proposed in 1957 and was eventually recognised by a Nobel
...
4.5 × 10¯² J. What is the magnitude of magnetic moment of the magnet?
A short bar magnet of magnetic moment m = 0.32 JT¯¹ is placed in a
...
A bar magnet of magnetic moment 1.5 J T¯¹ lies aligned with the
...
A short bar magnet has a magnetic moment of 0.48 J T¯¹. Give the

# NCERT: 12TH_PHYSICS_CH07_ALTERNATING_CURRENT_CH07_ALTERNATING_CURRENT_EN
...
We have so far considered direct current (dc) sources and circuits with de
...
reason for preferring use of ac voltage over de voltage is that ac voltages
...
apply Kirchhoff's loop rule ∑e(t) = 0 (refer to Section
...
well for both ac and de voltages. The voltage across a pure resistor
and the current through it, given by Eqs. (7.1) and (7.2) are
plotted as a function of time in Fig. 7.2. Note, in particular that
both vand i reach zero, minimum and maximum values at the
...
know, Joule heating is given by i²R and depends on i²
(which is always positive whether iis positive or negative)
...
inside the bracket. Since, i and Rare constants,
...
[7.5(b)]
Using the trigonometric identity, sin² ot =
1/2 (1-cos 2ot), we have < sin² wt >= (1/2) (1−<cos2wt>)
...
To express ac power in the same, NCER
...
peak current i by I = i /√2 = 0.707 im.

m

The average value of a function F (t) over a period Tis given by (F(t) = =[F(t)dt
...
Equation (7.9) gives the relation betweelished
...
current and ac voltage

and is similar to that in the de case. This shows the advantage of
...
amplitudes or the peak values un and im of these
oscillating quantities. Figure 7.4(a) shows the
voltage and current phasors and their
relationship at time t₁ for the case of an ac source
...
7.1. (b) Graph of v and

i versus wt.

voltage and current phasors on vertical axis, i.e., v sinotand im sinot,
...
the voltage across the source be v = vm sino t. Using
the Kirchhoff's loop rule, ∑ɛ(t) = 0, and since there
...
is the amplitude of the current. The quantity @ Lis
...
since the average of sin (2ot) over a complete cycle is zero.

Thus, the average power sup

cycle is zero.

supplied to an inductor over one complete
...
in a de circuit, current will flow for the short time
...
a capacitor in a de circuit will limit or oppose the
...
charge on the capacitor at any
...
where the amplitude of the oscillating current is i im = wCvm. We can rewrite

Um
(1/@C)

Comparing it to_im=vm/R for a purely resistive circuit, we find that
(1/C) plays the role of resistance. It is called capacitive reactance and
...
ohm (2). The capacitive reactance limits the
...
With ac source, the capacitor offers capacitative reactance (1/C)
...
Eq. (7.21). Further, let V₁, VR, Vc, and V represent the voltage across the
...
phasors V₁, VR, VC, and I, (b) Relation
between the phasors VL, VR, and (V₁ + V)
for the circuit in Fig. 7.10.

This relation is represented in Fig. 7.11(b). Since
Vc and V₁ are always along the same line and in
opposite directions, they can be combined into a single phasor (Vc + V₁)
which has a magnitude | Ucm - ULm. Since V is represented as the
hypotenuse of a right-triangle whose sides are V₨ and (Vc + V₁), the
...
Substituting the values of Urm' Ucm' , and ULm from Eq. (7.22) into the above
equation, we have
...
If X > X₁, & is positive and the circuit is predominantly capacitive.
...
(b) Graphs of u and i versus @t for a series LCR
...
of t (say, t₁, as done throughout this chapter)
...
with X = 1/@Cand X₁ = @L. So if @ is varied, then at a particular frequency

C

‰, X = X₁, and the impedance is minimum (Z = √R² + 0² = R).
...
vm/Rat resonance, the current amplitude for
...
Example 7.6 A resistor of 200 2 and a capacitor of 15.0 µF are
...
VR = IR = (0.755A)(200Ω) = 151V

V = I Xc = (0.755A)(212.3Ω) = 160.3 V

The algebraic sum of the two voltages, VR and V is 311.3 V which
...
We have seen that a voltage v = v sinot applied to a series RLC circuit
drives a current in the circuit given by i = i sin(wt + $) where
...
quantity cos is called the power factor. Let us discuss the following

cases:

Case (i) Resistive circuit: If the circuit contains only pure R, it is called
resistive. In that case $ = 0, cos q = 1. There is maximum power dissipation.
...
voltage and current is π/2. Therefore, cos ¢ = 0, and no power is dissipated
...
given by Eq. (7.30) where ¢ = tan¯¹ (X–X₁)/ R. So, may be non-zero in
...
X – X₁= 0, and ¢ = 0. Therefore, cos¢ = 1 and P = I2Z = I2 R. That is,
...
Solution (a) We know that P = I V cos¢ where cosø is the power factor.
...
along the applied voltage V and I perpendicular to the applied
voltage. I, as you have learnt in Section 7.7, is called the wattless
component since corresponding to this component of current, there
is no power loss. I is known as the power component because it is
...
we must completely neutralize the lagging wattless current I by an
equal leading wattless current I'. This can be done by connecting
a capacitor of appropriate value in parallel so that I₁ and I'q cancel
each other and P is effectively I V.

q

Example 7.8 A sinusoidal voltage of peak value 283 V and frequency
50 Hz is applied to a series LCR circuit in which
R = 3 Ω, L = 25.48 mH, and C = 796 µF. Find (a) the impedance of the
...
Since is negative, the current in the circuit lags the voltage
across the source.
...
coils called the primary coil has N, turns. The other coil is called the
...
primary and secondary windings. Let ø be the flux in each turn in the core
at time t due to current in the primary when a voltage v is applied to it.
Then the induced emf or voltage ɛ, in the secondary with N turns is
...
But &p = up. If this were not so, the primary current would be infinite
...
where u is the voltage across the secondary. Therefore, Eqs. (7.31) and
...
primary (N > N), the voltage is stepped up (V > V). This type of
arrangement is called a step-up transformer. However, in this arrangement,
there is less current in the secondary than in the primary (Np/N｡< 1 and I｡
<I). For example, if the primary coil of a transformer has 100 turns and
the secondary has 200 turns, N/N = 2 and Np/N=1/2. Thus, a 220V
input at 10A will step-up to 440 V output at 5.0 A.

If the secondary coil has less turns than the primary (N < N),
we have a step-down transformer. In this case, V¸ < V„ and I¸ > I. That
...
(I2R). In high current, low voltage windings, these are minimised by
using thick wire.
...
consequently, the I²R loss is cut down). It is then transmitted over long
...
and utility poles before a power suppl supply of 240 V reaches our homes.

NCE

SUMMARY

### 1. An alternating voltage v = v sin ot_applied to a resistor R drives a

ng voltage = sin(

m

current i = i sinot in the resistor, im
...
### 2. For an alternating current i = i sin at passing through a resistor R, the
average power loss P (averaged over a cycle) due to joule heating is
(1/2)i² R. To express it in the same form as the dc power (P = I2R), a
...
### 3. An ac voltage v = vm sin ot applied to a pure inductor L, drives a current
in the inductor i = im sin (ωt – π/2), where im = Um/XL. X₁ = OL is called
...
### 4. An ac voltage v = vm sinot applied to a capacitor drives a current in the
capacitor: i = i sin (ωt + π/2). Here,
...
### 5. For a series RLC circuit driven by voltage v = v sin ot, the current is
...
The term coso is called the power factor.

6. In a purely inductive or capacitive circuit, coso = 0 and no power is
...
coil of N turns and a secondary coil of N｡ turns. If the primary coil is
...
total voltage across RC combination is VRC
VR + V since V is π/2 out of phase of VR
...
8. In a RLC circuit, resonance phenomenon occur when X₁ = Xc or
ωο =
For resonance to occur, the presence of both Land C
elements in the circuit is a must. With only one of these (Lor C)
...
A 60 µF capacitor is connected to a 110 V, 60 Hz ac supply. Determine
the rms value of the current in the circuit.
...
A charged 30 µF capacitor is connected to a 27 mH inductor. What is
...
A series LCR circuit with R = 20 Ω, L = 1.5 Hand C = 35 µF is connected

# NCERT: 12TH_CHEMISTRY_CH04_D_AND_F_BLOCK_ELEMENTS_CH04_D_AND_F_BLOCK_ELEMENTS_EN
...
mercury of group 12 have full d¹º configuration in their
...
is (n-1)d¹-10ns¹-2except for Pd where its electronic configuration is 4d¹º5s°.
...
configuration is 3d¹º4s¹ and not 3d4s². The ground state electronic
...
are represented by the general formula (n-1)d¹ºns². The orbitals in
...
surrounding them. In some respects, ions of a given ď configuration
...
other hand, zinc atom has completely filled d orbitals (3d¹º) in its
...
### 4.1 Silver atom has completely filled d orbitals (4d¹º) in its ground state.
...
In general, gene ions of the same charge in a given series show progressive
...
Enthalpy of atomisation, ∆H°/kJ mol¯¹
...
Why do the transition elements exhibit higher enthalpies of Example 4.2
...
the cases, ions have d configuration. Similar breaks occur at
...
exchange energy at dº configuration. Mn⁺ has 3d 4s¹ configuration and
configuration of Cr⁺ is ď, therefore, ionisation enthalpy of Mn⁺ is lower
than Cr⁺. In the same way, Fe2+ has dº configuration and Mn2+ has 3d5
...
The lowest common oxidation state of these metals is +2. Το
...
have the d and d¹º configurations respectively. The value for Zn is
correspondingly low as the ionisation causes the removal of one 4s
electron which results in the formation of stable d¹º configuration.
...
electron from the d³ (Mn²⁺) and d¹º (Zn²⁺) ions. In general, the third
...
to the sum of the s and d electrons upto manganese (TiO2, VO2*,
CrO42, MnO4) followed by a rather abrupt decrease in stability of
higher oxidation states, so that the typical species to follow are Fem,
Co, Ni, Cu, Zn".
...
that their oxidation states differ from each other by unity, e.g., VI, VIII,
VV, VV. This is in contrast with the variability of oxidation states of non
...
ligands capable of π-acceptor character in addition to the o-bonding.
...
The unique behaviour of Cu, having a positive Eº, accounts for its
inability to liberate H₂ from acids. Only oxidising acids (nitric and hot
concentrated sulphuric) react with Cu, the acids being reduced. The
high energy to transform Cu(s) to Cu²+(aq) is not balanced by its hydration
...
Why is Cr²+ reducing and Mn³t oxidising when both have d² configuration? Example 4.4

3+

2+

Cr2+ is reducing as its configuration changes from d⁺ to ď³, the latter Solution
...
reason for this? (Hint: consider its high ∆H° and low ∆hydH°)
...
Table 4.4: Thermochemical data (kJ mol¹) for the first row Transition
...
filled d¹º configuration in Zn²⁺ are related to their Eº values, whereas Eº
for Ni is related to the highest negative AhydH°.

2) shows

An examination of the E°(M³+/M²*) values (Table 4.2) shows the varying
trends. The low value for Sc reflects the stability of Sc³⁺ which has a
noble gas configuration. The highest value for Zn is due to the removal
of an electron from the stable d¹º configuration of Zn2+. The
comparatively high value for Mn shows that Mn²*(d) is particularly
...
of V²+ (half-filled t₂g level, Unit 5).
...
Table 4.5 shows the stable halides of the 3d series of transition metals.
...
known except the iodide. In this case, Cu²⁺ oxidises I to 12:
...
more negative AhyaH° of Cu2+ (aq) than Cut, which more than
...
known, although ferrates (VI)(FeO₄)²¯, are formed in alkaline media but
they readily decompose to Fe2O3 and O2. Besides the oxides, oxocations
stabilise V as VO2+, V¹Vas VO²⁺ and Ti™ as TiO2⁺. The ability of oxygen
...
ions are known for V, Cr, Mn, Mn and MnVII.
...
passive to dilute non oxidising acids at room temperature. The Eº values
for M2+/M (Table 4.2) indicate a decreasing tendency to form divalent
cations across the series. This general trend towards less negative Eº
values is related to the increase in the sum of the first and second
ionisation enthalpies. It is interesting to note that the Eº values for Mn,
...
filled d subshell (d¹º) in zinc are related to their E˚ values; for nickel, Eº
...
An examination of the Eº values for the redox couple M³+/M²⁺ (Table
4.2) shows that Mn³t and Co³+ ions are the strongest oxidising agents
in aqueous solutions. The ions Ti²⁺, V2+ and Cr²⁺ are strong reducing
...
Example 4.6 For the first row transition metals the Eº values are:
...
Solution The E° (M²⁺/M) values are not regular which can be explained from

the irregular variation of ionisation enthalpies (∆₁H₁ + ∆₁H₂) and also
the sublimation enthalpies which are relatively much less for
manganese and vanadium.

Example 4.7 Why is the Eº value for the Mn³+/Mn2+ couple much more positive
than that for Cr3+/Cr2+ or Fe3+/Fe2+? Explain.

Solution Much larger third ionisation energy of Mn (where the required change
is ď to d²) is mainly responsible for this. This also explains why the
...
where n is the number of unpaired electrons and u is the magnetic
...
ď configuration (five unpaired electrons). The magnetic moment, u is
...
energy dorbital, the energy of excitation corresponds to the frequency
...
left to right: V4+,V3+,Mn2+,Fe3+,Co2+,Ni2+and Fe3+,C Cu2+ .
...
like H, C or N are trapped inside the cryst crystal lattices of metals. They are
usually non stoichiometric and are neither typically ionic nor covalent,
Compounds for example, TiC, Mn₁N, Fe3H, VH0.56 and TiH1.7, etc. The formulas
...
Example 4.9 What is meant by ‘disproportionation' of an oxidation state? Give an
...
Mn2O7. Beyond group 7, no higher oxides of iron above Fe2O3_are
...
Thus, Mn2O7 gives HMnO4 HMnO4 and CrO3 gives H2CrO4 and H2Cr2O7.
V2O5 is, however, amphoteric though mainly acidic and it gives VO₄ as
...
to give VO and VO respectively. The well characterised CrO is basic
...
2 CrO42 + 2H* → Cr2O7²¯ + H2O
Cr2O7² + 2 OH → 2 CrO4²¯ + H₂O
...
Cr2O7²¯ + 14 H* + 6 Fe2+ → 2 Cr³+ + 6 Fe3+ + 7 H₂O
...
2Mn²⁺ + 5S2Og²¯ + 8H2O → 2MnO4¯ + 10SO42¯ + 16H+
...
101¯ + 2MnO4¯ + 16H⁺ → 2Mn2+ + 8H2O + 512

(b) Fe2+ ion (green) is converted to Fe³+ (yellow):
...
8MnO4 + 3S2O32- + H2O -> 8MnO2 + 6SO₄²¯ + 2OH¯
...
configuration with 6s² common but with variable occupancy of 4f level
...
form 4f" (n = 1 to 14 with increasing atomic number).
...
common +3 state. The E° value for Ce+/ Ce³+ is + 1.74 V which
...
the presence of f electrons. Neither La³⁺ nor Lu³t ion shows any colour
...
than the fº type (La³+ and Ce⁺⁺) and the f 14 type (Yb2+ and Lu³*) are
...
600 kJ moľ¯¹, the second about 1200 kJ mol¯¹ comparable with those
...
they behave more like aluminium. Values for E° for the half-reaction:
...
### 4.6 The Actinoids The actinoids include the fourteen elements from Th to Lr. The names,
...
are related to the stabilities of the fº, f' and f¹¹ occupancies of the 5f
orbitals. Thus, the configurations of Am and Cm are [Rn] 5f 77s² and
[Rn] 5f76d7s². Although the 5f orbitals resemble the 4f orbitals in their
...
There is a gradual decrease in the size of atoms or M³+ ions across the
...
Example 4.10 Name a member of the lanthanoid series which is well known
...
### 4.10 Actinoid contraction is greater from element to element than
...
following d electron configurations in the ground state of their atoms : 3ď,
3d, 3d and 3d¹?
...
### 4.17 For M2+/M and M³+/M²+ systems the Eº values for some metals are as follows:
...
(i) the stability of Fe3+ in acid solution as compared to that of Cr³+ or Mn³+ and
(ii) the ease with which iron can be oxidised as compared to a similar process
for either chromium or manganese metal.

### 4.18 Predict which of the following will be coloured in aqueous solution? Ti³+, V³+,
Cut, Sc³+, Mn2+, Fe3+ and Co2t. Give reasons for each.
...
(i) Of the d² species, Cr²+ is strongly reducing while manganese(III)
is strongly oxidising.

(ii) Cobalt(II) is stable in aqueous solution but in the presence
...
### 4.24 Calculate the number of unpaired electrons in the following gaseous ions: Mn³+,
Cr³+, V³+ and Ti³t. Which one of these is the most stable in aqueous solution?
...
### 4.30 Which is the last element in the series of the actinoids? Write the electronic
...
### 4.31 Use Hund's rule to derive the electronic configuration of Ce³+ ion, and calculate
...
4.34 Write the electronic configurations of the elements with the atomic numbers
...
### 4.36 Write down the number of 3d electrons in each of the following ions: Ti²⁺+, V2+,
Cr³+, Mn2+, Fe2+, Fe3+, Co2+, Ni²⁺ and Cu²⁺. Indicate how would you expect the five
...
### 4.2 In the formation of metallic bonds, no eletrons from 3d-orbitals are involved
...
degree of stability of different 3d-configurations (e.g., dº, d, d¹º are
...
Reason: d⁺ → d³ occurs in case of Cr2+ to Cr3+

But d° → d occurs in case of Fe2+ to Fe3+

In a medium (like water) d³ is more stable as compared to dº (see CFSE)
...
### 4.10 The 5f electrons are more effectively shielded from nuclear charge. In other

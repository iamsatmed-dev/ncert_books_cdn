# NCERT: 12TH_CHEMISTRY_CH02_ELECTROCHEMISTRY_CH02_ELECTROCHEMISTRY_EN
...
define resistivity (p), conductivity
(k) and molar conductivity (A) of
ionic solutions;
...
define Am (molar conductivity at
...
of Zn2+ and Cu2+ ions is unity (1 mol dm³)*.
...
the
their 
salient features in the following pages.
...
and we have Ecell = Eright - Eleft = EAg Ag - Ecu²+ |Cu
...
then the cell potential is equal to standard electrode potential, EOR of
the given half-cell.

E° = ER - EL

As EL for standard hydrogen electrode is zero.
E° = ER - 0 = E°R

The measured emf of the cell:

Pt(s) | H₂(g, 1 bar) | H⁺ (aq, 1 M) || Cu2+ (aq, 1 M) | Cu
...
Pt(s) | H₂(g, 1 bar) | H* (aq, 1 M) || Zn2+ (aq, 1M) | Zn
is -0.76 V corresponding to the standard electrode potential
half-cell reaction:
...
Left electrode: Zn(s) → Zn²* (aq, 1 M) +2e
...
With half-cell reaction: ½ Br₂(aq) + e → Br¯(aq)
...
product, equilibrium constant and other properties
ermodynamic
and for potentiometric titrations.

Intext Questions

ctrode potential of the system

### 2.1 How would you determine the standard electrode
Mg2+ | Mg?
...
E(Mn+/M) has already been defined, Ris gas constant (8.314 JK¯¹ moľ¯¹),
...
Table 2.1: Standard Electrode Potentials at 298 Κ
...
### 1. A negative Eº means that the redox couple is a stronger reducing agent than the H+/H₂ couple.
2. A positive Eº means that the redox couple is a weaker reducing agent than the H+/H₂ couple.
...
The cell potential, E(cell) = E(Cu2+/Cu)
...
Ecu²/ca) - In [Cu(aq)] - Eiza²+/Zn) + In [Zn (aq)]
...
E
E(
cell) depends on the concentration of both Cu2+
...
The cell reaction is Ni(s) + 2Ag+(aq) → Ni²+(aq) + 2Ag(s)
...
The cell can be written as Mg | Mg2+(0.130M) || Ag+ (0.0001M) | Ag Solution
...
of charge passed and △₁G is the Gibbs energy of the reaction, then
AG = nFE (cell)

It may be remembered that E

(2.15)
E(cell) is an intensive parameter but ∆G
...
Thus, from the measurement of E(cell) we can obtain an important
thermodynamic quantity, A₁Gº, standard Gibbs energy of the reaction.
...
Solution AG° = NFE E(cell)

n in the above equation is 2, F = 96487 C mol¯¹ and E(cell) = 1.1 V
...
2Fe3+ (aq) + 21 (aq) → 2Fe2+ (aq) + I2 (s) has Eell = 0.236 V at 298 Κ.
...
which in terms of SI base units is equal to (kg m³)/(S³ A²). It can be
...
The constant of proportionality, p (Greek, rho), is called resistivity
...
of cross section is one m². It can be seen that:

2

1 Ωm = 100 Ω cm or 1 Ω cm = 0.01 Ωm
...
'S' and is equal to ohm¯¹ (also known as mho) or 2¯¹. The inverse of
resistivity, called conductivity (specific conductance) is represented by
the symbol, к (Greek, kappa). IUPAC has recommended the use of term
...
Sm¯¹ but quite often, k is expressed in S cm¯¹. Conductivity of a
material in S m¯¹ is its conductance when it is 1 m long and its area
of cross section is 1 m². It may be noted that 1 S cm = 100 S m¯¹.
...
(3.5 × 105 Sm¯¹). When electrolytes are dissolved in water, they furnish
...
electrodes is a column of length 1 and area of cross section A. The
...
cross-section and has the dimension of length¯¹ and can be calculated
...
Concentration/Molarity Conductivity
...
conductivity denoted by the symbol Am (Greek, lambda). It is related
...
(2.21)
In the above equation, if k is expressed in S m¯¹ and the concentration,
c in mol m³ then the units of Am are in S m² mol¯¹. It may be noted that:

-3

1 mol m¯³ = 1000(L/m³) × molarity (mol/L), and hence
...
If we use S cm¯¹ as the units for к and mol cm³, the units of
concentration, then the units for Am are S cm² mol¯¹. It can be calculated
...
1 S cm²mol-1 = 10¯4 S_m²mol-1.

Resistance of a conductivity cell filled with 0.1 mol L¯¹ KCl solution is Example 2.4
...
of 0.02 mol L¯¹ KCl solution. The conductivity of 0.1 mol L¯¹ KCl
...
= 1.29 S/m × 100 Ω = 129 m¯¹ = 1.29 cm¯¹

-1

Conductivity of 0.02 mol L¯¹ KCl solution = cell constant / resistance
...
Example 2.5 The electrical resistance of a column of 0.05 mol L¯¹ NaOH solution of
diameter 1 cm and length 50 cm is 5.55 × 103 ohm. Calculate its
resistivity, conductivity and molar conductivity.

Solution A = πr² = 3.14 × 0.52 cm² = 0.785 cm² = 0.785 × 10^4 m²
...
G = = K (both A and I are unity in their appropriate units in
mor cm)
...
in kon dilution of a solution is more
than compensated by increase in its
volume. Physically, it means that at a
given concentration, A㎜ can be defined
...
Fig. 2.6: Molar conductivity versus c½ for acetic
...
For strong electrolytes, A increases slowly with dilution and can be
...
It can be seen that if we plot (Fig. 2.6) Am against
c1/2, we obtain a straight line with intercept equal to Am and slope
...
Show that a plot between Am and c1/2 is a straight line. Determine the
values of Am and A for KCl.
...
Kohlrausch examined Am values for a number of strong electrolytes
and observed certain regularities. He noted that the difference in Am of
...
if a Nat and a ci are limiting molar conductivity of the sodium and chloride
...
Here, λ and λ are the limiting molar conductivities of the cation
and anion respectively. The values of λ for some cations and anions at
...
higher concentrations and hence for such electrolytes, the change in Am
...
1 mol of electrolyte. In such cases Am increases steeply (Fig. 2.6) on
dilution, especially near lower concentrations. Therefore, Am cannot be
obtained by extrapolation of Am to zero concentration. At infinite dilution
...
that it cannot be measured accurately. Therefore, Am for weak electrolytes
...
then it can be approximated to the ratio of molar conductivity Am at the
concentration c to limiting molar conductivity, Am. Thus we have:
...
calculate Am for any electrolyte from the λº of individual ions. Moreover,
for weak electrolytes like acetic acid it is possible to determine the value
of its dissociation constant once we know the Am and Am at a given
concentration c.

Example 2.7 Calculate Am for CaCl₂ and MgSO4 from the data given in Table 3.4.
...
119.0 S cm² mol¯¹ + 2(76.3) S cm² mol¯¹

(119.0 + 152.6) S cm² mol¯¹
271.6 S cm² mol-1
...
Example 2.8 Am for NaCl, HCl and NaAc are 126.4, 425.9 and 91.0 S cm² mol¯¹

respectively. Calculate Aº for HAc.
...
Example 2.9 The conductivity of 0.001028 mol L¯¹ acetic acid is 4.95 × 105 S cm¯¹.
...
### 2.8 Suggest a way to determine the Am value of water.

### 2.9 The molar conductivity of 0.025 mol L¯¹ methanoic acid is 46.1 S cm² moľ¯¹.

Calculate its degree of dissociation and dissociation constant. Given λº(H*)
= 349.6 S cm² mol¯¹ and λº (HCOO¯) = 54.6 S cm² mol¯¹.
...
coulometers are now obsolete and we now have constant current (1)
sources available and the quantity of electricity Q, passed is given by
Q = It

is in coloumbs when I is in ampere and t is in second.
...
NA × 1.6021 × 10-19 C = 6.02 × 1023 mol¯¹ × 1.6021 × 10-19
C = 96487 C mol¯¹

This quantity of electricity is called Faraday and is represented by
the symbol F.

For approximate calculations we use 1F ≈ 96500 C mol¯¹.
...
(2.32)
It is obvious that one mole of *Mg2+ Mg2+ and Al³+ require 2 mol of electrons
...
t = 600s charge = current × time = 1.5 A × 600 s = 900 C
...
sodium metal and Cl₂ gas. Here we have only one cation (Na⁺) which is
reduced at the cathode (Na⁺ + e → Na) and one anion (CI) which is
oxidised at the anode (CI → ½Cl₂ + e¯). During the electrolysis of aqueous
sodium chloride solution, the products are NaOH, Cl2 and H2. In this
case besides Na⁺ and Cľ ions we also have H⁺ and OH¯ ions along with
the solvent molecules, H₂O.
...
The reaction with higher value of Eº is preferred and therefore, the
...
2H₂O 2H₂O (l) → O2 O₂ (g) + 4H+(aq) + 4e E

E(cell) = 1.23 V

(2.37)

The reaction at anode with lower value of Eº is preferred and
...
NaCl(aq) + H2O(l) → Na*(aq) + OH¯(aq) + 2H2(g) + 2Cl2(g)
...
2SO4 (aq) → S₂O (aq) + 2e¯ E(cell) = 1.96 V
...
2.12 Consider the reaction: Cr2O72 + 14H+ + 6e¯ → 2Cr3+ +
...
Anode:
Pb(s) + SO42-(aq) → PbSO₄(s) + 2e¯

Cathode: PbO2(s) + SO42-(aq) + 4H+(aq) + 2e- → PbSO4 (s) + 2H2O (1)
...
Cd (s) + 2Ni(OH)3 (s) → CdO (s) + 2Ni(OH)2 (s) + H₂O (1)
...
oxidation: 2Fe²+(aq) + 2H2O(l) + 2O2(g) → Fe2O3(s) +
...
(which is believed to be available from H₂CO₃ formed due to dissolution
...
Cathode: O2(g) + 4 H*(aq) + 4 e¯ → 2H₂O (1) EH+10, |H,0 = 1.23 V
The overall reaction being:

2Fe(s) + O2(g) + 4H+(aq) → 2Fe² +(aq) + 2 H2O (1) E(cell) =1.67 V
...
anode (E(cell) = E°cathode Eanode). The standard potential of the cells are
related to standard Gibbs energy (ArG° = −nFE(cell)) and equilibrium constant
(∆rG° = − RT In K) of the reaction taking place in the cell. Concentration dependence
...
of the electrolyte, nature of solvent and temperature. Molar conductivity, Am, is
defined by = к/с where с is the concentration. Conductivity decreases but molar
...
Calculate the AG°and equilibrium constant of the reactions.
...
(iii) Sn(s) | Sn2+(0.050_M) ||H*(0.020 M)|H2(g) (1 bar)|Pt(s)
...
Zn(s) + Ag2O(s) + H₂O(l) → Zn2+(aq) + 2Ag(s) + 2OH(aq)
...
102 × к/S m¯¹ 1.237 11.85 23.15 55.53 106.74
Calculate Am for all concentrations and draw a plot between Am and c½.
Find the value of Am

### 2.11 Conductivity of 0.00241 M acetic acid is 7.896 × 10-5 S_cm¯¹. Calculate its
molar conductivity. If A for acetic acid is 390.5 S cm² mol¯¹, what is its
...
respectively ar are connected in series. A steady current of 1.5 amperes was
